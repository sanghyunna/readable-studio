...
//   node --experimental-strip-types scripts/backfill-failed-runs-with-artifacts.ts --data-dir /path/to/.readable-studio --dry-run
//   node --experimental-strip-types scripts/backfill-failed-runs-with-artifacts.ts --data-dir /path/to/.readable-studio
//
...
function parseArgs(argv: string[]): { dataDir: string; dryRun: boolean } {
  let dataDir = process.env.READABLE_DATA_DIR?.trim() || './.readable-studio';
  let dryRun = false;

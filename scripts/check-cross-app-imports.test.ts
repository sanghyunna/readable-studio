...
const registry: AppDirectoryRegistry = {
  packageNameByDirectory: new Map([
    ["daemon", "@readable-studio/daemon"],
    ["web", "@readable-studio/web"],
  ]),
};
...
test("cross-app import check rejects another app's package name", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "import type { RunRecord } from '@readable-studio/daemon/src/server.ts';",
      "const mod = await import('@readable-studio/daemon');",
      "const legacy = require('@readable-studio/daemon/dist/cli.js');",
    ].join("\n"),
    registry,
  );
...
test("cross-app import check rejects another app's package name in require.resolve", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "const daemonManifest = require.resolve('@readable-studio/daemon/package.json');",
    registry,
  );

  assert.equal(violations.length, 1);
  assert.equal(violations[0]?.specifier, "@readable-studio/daemon/package.json");
  assert.equal(violations[0]?.targetApp, "daemon");
  assert.equal(violations[0]?.lineNumber, 1);
});
...
test("cross-app import check rejects createRequire-based cross-app resolution", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      "const directDaemonManifest = createRequire(import.meta.url).resolve('@readable-studio/daemon/package.json');",
      "const aliasDaemonManifest = makeRequire(import.meta.url).resolve('@readable-studio/daemon/package.json');",
      "const nodeRequire = nodeModule.createRequire(import.meta.url);",
      "const daemonCli = nodeRequire('@readable-studio/daemon/dist/cli.js');",
      "const daemonPackageJson = nodeRequire.resolve('@readable-studio/daemon/package.json');",
    ].join("\n"),
    registry,
  );
  ...
  assert.deepEqual(
    ...
    [
      "@readable-studio/daemon/package.json",
      "@readable-studio/daemon/package.json",
      "@readable-studio/daemon/dist/cli.js",
      "@readable-studio/daemon/package.json",
    ],
  );
  assert.deepEqual(
...
test("cross-app import check rejects CommonJS node:module namespace createRequire resolution", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      'nodeRequire.resolve("@readable-studio/daemon/package.json");',
    ].join("\n"),
    registry,
  );
...
test("cross-app import check rejects cross-app imports from app-owned mjs entrypoints", () => {
  ...
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "import { startDaemon } from '@readable-studio/daemon/src/server.ts';",
    registry,
  );
...
test("cross-app import check allows packages, same-app relatives, and externals", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      "import type { ChatRunEvent } from '@readable-studio/contracts';",
...
test("cross-app import check ignores quoted snippets and comments", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "const snippet = \"import { x } from '@readable-studio/daemon/src/server.ts';\";",
      "// import { y } from '@readable-studio/daemon/src/server.ts';",
      "/*",
      "require('@readable-studio/daemon/dist/cli.js');",
      "*/",
    ].join("\n"),
    registry,
  );
...
test("cross-app import check allows allowlisted packaged -> desktop main export", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "import { applyOsLocaleSwitch, createSplashWindow } from '@readable-studio/desktop/main';",
    { packageNameByDirectory: new Map([["packaged", "@readable-studio/packaged"], ["desktop", "@readable-studio/desktop"]]) },
  );

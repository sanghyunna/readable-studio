/**
 * readable-landing-deck — input schema.
 *
 * Sister skill to `readable-landing`. Produces a single-file slide
...
import type { MixedText, BrandBlock, ImageryConfig } from '../readable-landing/schema';
...
export interface CoverSlide {
  kind: 'cover';
  /** Eyebrow above the title — `'Readable Studio · Vol. 01'`. */
  eyebrow: string;
...
export interface EndSlide {
  kind: 'end';
  /** The huge kicker — `'Readable Studio.'`. */
  mega: MixedText;
...
export interface ReadableLandingDeckInputs {
  $schema?: string;
  brand: BrandBlock;
  /** Deck-wide title shown in the HUD — `'Readable Studio · Vol. 01'`. */
  deck_title: string;
  slides: Slide[];

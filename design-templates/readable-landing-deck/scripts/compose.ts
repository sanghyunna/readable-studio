...
// allow: SIZE_OK - indivisible template composer for one standalone HTML deck.
/**
 * readable-landing-deck — slide deck composer.
 *
 ...
 * stylesheet from the sister `readable-landing` skill, then layers
...
const SISTER_STYLES = resolve(SKILL_ROOT, '..', 'readable-landing', 'styles.css');
...
/* ------------------------------------------------------------------ *
 * deck-specific stylesheet (layered on top of readable-landing CSS).
 *
...
function chromeStrip(brand: ReadableLandingDeckInputs['brand'], deckTitle: string): string {
...
function footStrip(idx: number, total: number, brand: ReadableLandingDeckInputs['brand']): string {
...
function renderSlide(
  ...
  i: number,
  total: number,
  inputs: ReadableLandingDeckInputs,
  assets: string,
): string {
...
(function () {
  ...
  function applySlide(n) {
    ...
    /* load-bearing: .slide.active is read by Readable Studio's host bridge
  ...
  /* Host-driven navigation: Readable Studio's host bridge classifies this deck
     ...
     Two cooperating handlers keep the deck in sync with the host:
       1. A readable:slide message listener routes host nav through go() and
  ...
  addEventListener('message', function (e) {
    var data = e && e.data;
    if (!data || data.type !== 'readable:slide') return;
...
export function renderDeck(inputs: ReadableLandingDeckInputs, baseCss: string): string {
...
async function main(): Promise<void> {
  ...
  const inputs = JSON.parse(inputsRaw) as ReadableLandingDeckInputs;

...
```bash
pnpm --filter @readable-studio/web build 2>&1 | tail -20 > /tmp/web-baseline-build.txt
```
...
```bash
pnpm --filter @readable-studio/contracts test critique.test.ts
```
...
```bash
pnpm --filter @readable-studio/contracts test sse.test.ts
```
...
```bash
pnpm --filter @readable-studio/daemon test parser.test.ts
```
...
import type { PanelEvent, PanelistRole } from '@readable-studio/contracts/critique';
...
```bash
pnpm --filter @readable-studio/daemon test scoreboard.test.ts
```
...
import type { PanelistRole } from '@readable-studio/contracts/critique';
...
```ts
import type { CritiqueConfig, RoundDecision } from '@readable-studio/contracts/critique';
...
```ts
import type { FallbackPolicy } from '@readable-studio/contracts/critique';
...
```bash
pnpm --filter @readable-studio/daemon test migrations.test.ts
```
...
```bash
pnpm --filter @readable-studio/daemon test orchestrator.test.ts
```
...
import { panelEventToSse } from '@readable-studio/contracts/sse';
import { parseCritiqueStream } from './parser';
...
```ts
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig, PROTOCOL_VERSION } from '@readable-studio/contracts/critique';
...
import { type CritiqueConfig, PROTOCOL_VERSION } from '@readable-studio/contracts/critique';
...
```bash
pnpm --filter @readable-studio/web test discovery.test.ts
```
...
import type { CritiqueSseEvent } from '@readable-studio/contracts/sse';
import type { PanelistRole } from '@readable-studio/contracts/critique';

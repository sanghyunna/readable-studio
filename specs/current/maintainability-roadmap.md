...
This document captures maintainability risks in the `apps/web` + `apps/daemon` architecture that supports Readable Studio's complete workflow: source text, AI generation, direct editing, and standalone HTML export.

The architectural boundary stays unchanged:

- `apps/web`: Next.js document workspace, sandboxed preview, and direct-edit UI.
- `apps/daemon`: local runtime/backend for SQLite, `.readable-studio` filesystem state, AI agent CLI processes, plugins, export, and SSE streaming.
...
| R13 | P2 | Windows runtime behavior is a recurring risk. | Daemon uses filesystem paths, SQLite native bindings, shell/process behavior, and process signals. | Windows versions, shells, permissions, native addon ABIs, and process trees can differ. | Use Node path APIs consistently, avoid shell string composition, isolate Windows-specific process logic, and validate the Windows 10/11 x64 portable artifact. |
...
| R16 | P3 | Operational documentation is incomplete. | Local daemon behavior depends on transient ports, `.readable-studio`, portable `ReadableStudioData`, agent CLIs, runtime logs, and recovery flows. | Onboarding and support costs rise; troubleshooting relies on oral knowledge. | Document daemon architecture, API/SSE contract, task lifecycle, data layout, agent dependency checks, export behavior, and common recovery procedures. |
...
| W3 | Partial | Migrate project-owned code to TypeScript | R1 | W2 for highest-value shared types | Daemon source and tests have most `@ts-nocheck` suppressions removed. High-conflict daemon core files are deferred for follow-up to avoid blocking parallel PRs: `apps/daemon/src/server.ts`, `apps/daemon/src/agents.ts`, `apps/daemon/src/projects.ts`, `apps/daemon/src/runs.ts`, and `apps/daemon/src/cli.ts`. Remaining work is to migrate those files, restore full daemon typecheck coverage, and rerun `pnpm --filter @readable-studio/daemon typecheck`, `pnpm typecheck`, and `pnpm guard`. |
...
| W11 | Partial | Harden Windows runtime behavior | R13 | W4, W6, W5 | Some process and path hardening exists, including shared platform command invocation and Windows command-line budget checks for agent CLIs. Remaining work is to formalize platform-specific process handling, path normalization rules, and supported-platform package validation. |

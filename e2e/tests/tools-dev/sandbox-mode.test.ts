...
describe('tools-dev sandbox mode smoke', () => {
  test('launches web/daemon with READABLE_SANDBOX_MODE=1 and a sandboxed agent spawn', async () => {
    ...
    await suite.with.toolsDev(
      async ({ status, webUrl }) => {
        ...
        expect(capture.env.READABLE_SANDBOX_MODE).toBe('1');
        expect(capture.env.READABLE_DATA_DIR).toBe(suite.dataDir);
        expect(capture.env.READABLE_PROJECT_ID).toBe(project.project.id);
        expectPathUnder(capture.env.READABLE_AGENT_HOME, roots.agentHomeDir, 'READABLE_AGENT_HOME');
      ...
      {
        env: {
          READABLE_E2E_SANDBOX_CAPTURE_PATH: capturePath,
...
function emitRun(promptText) {
  if (emitted) return;
  emitted = true;
  const capturePath = process.env.READABLE_E2E_SANDBOX_CAPTURE_PATH;
  if (capturePath && process.env.READABLE_TOOL_TOKEN) {
    const env = {};

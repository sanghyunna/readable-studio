...
Previously, `apps/daemon/src/cli.ts` statically imported `server.ts`. Because ES modules execute top-level code during import, client-only commands also evaluated daemon startup globals, including `READABLE_DATA_DIR` resolution. A bad runtime data directory could therefore fail research search before the CLI even sent its HTTP request.

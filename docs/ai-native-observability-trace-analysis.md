...
This document records how to read the new Readable Studio Langfuse trace shape for
...
- PR: https://github.com/nexu-io/readable-studio/pull/3714
- Issue: https://github.com/nexu-io/readable-studio/issues/3733
...
content should remain in Readable Studio object storage and be referenced by
...
Raw sensitive content belongs in Readable Studio object storage with access control,
...
Issue #3733 targets the first durable bridge between Readable Studio-owned object
...
Readable Studio-owned project storage today and to a ProjectStorage or
...
uses Langfuse-safe manifest fields and `readable-studio://objects/...` references rather than
...
| Storage reference | Emits stable `readable-studio://objects/...` references. | Treats `storage_ref` as an Readable Studio registry reference, future-compatible with ProjectStorage/S3 backends. | No conflict; current refs should remain non-dereferenceable from Langfuse. |
...
1. Make the manifest builder resolve through a concrete Readable Studio object
...
4. Preserve `storage_ref` as a stable, non-secret reference and require any
   future download or preview access to go through Readable Studio-controlled
   authorization.
...
1. Store complete raw/redacted prompt payloads in Readable Studio object storage and

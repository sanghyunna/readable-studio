...
- New sample content is a maintained fixture for an Readable Studio capability, minimal enough to support the test, and stored under the [first-party fixture rule](#first-party-fixture-rule).
...
Domain-specific UI or content (a customer vertical, marketing experiment, unrelated rendering demo, arbitrary product page) is out of scope unless it is explicitly a first-party Readable Studio fixture and the PR explains which Readable Studio capability it validates.
...
A generally reasonable direction (e.g. "add Arabic/RTL coverage") is **not** sufficient justification to keep an unsalvageable PR open. Close with a comment that names which condition was met and asks for a fresh PR that uses the actual Readable Studio app and harness. If the contributor disputes the close, escalate to another maintainer rather than reopening unilaterally.
...
- Readable Studio app/mode/source/stamp details hard-coded inside generic `packages/sidecar` or `packages/platform` code.
...
- `apps/packaged` — packaged Electron entry, packaged sidecar startup, runtime integration, `readable-studio://` entry glue.
...
- `packages/sidecar-proto` — Readable Studio sidecar business protocol; app/mode/source constants, namespace validation, stamp fields/flags, IPC message schema, status shapes, error semantics.
...
- Adds tests, fixtures, scripts, or UI for a separate product/domain that does not exercise an existing Readable Studio feature.
...
- The skill supports design work in Readable Studio — artifact generation, visual design, layout, branding, design-system application, interaction design, accessibility review, design critique.
- It follows the structure and metadata conventions of existing first-party skills, including the skills protocol front-matter (e.g. `readable.craft.requires` when relevant).
- Instructions are general-purpose enough for Readable Studio workflows, not a single unrelated external service or business process.
...
- It adds broad external integrations or credentials not necessary for a design-focused Readable Studio workflow.
...
- The slug is short and stable enough to be referenced by skills via `readable.craft.requires`.
- At least one shipping skill opts into it via `readable.craft.requires`, OR the PR explains why the reference will be opted into in a follow-up and lists the candidate skills.
...
- **Domain-specific app smuggled in as tests.** A PR adding tests for a "Quranic Arabic Learning App" with selectors like `#dashboard`, `#exercise`, `.vocabulary-item`, `.arabic-word` when those surfaces are not part of Readable Studio — violates the "tests target real routes/DOM/APIs" rule.
- **Standalone-app E2E URLs.** Playwright tests hard-coding `http://localhost:17573/index.html` instead of using the configured Readable Studio app flow — violates the "use the existing harness, base URL, and lifecycle conventions" rule.
...
- **Ad hoc launcher script.** A new root script that hard-codes `~/projects/readable-studio`, assumes a user-specific tool such as `fnm`, or bypasses `pnpm tools-dev` — violates the "scripts use the documented control plane" rule and recreates a [Forbidden surface](#2-forbidden-surfaces).

A reasonable underlying intent (e.g. "we need RTL or Arabic coverage") does not justify keeping such a PR open. Ask for a fresh PR that exercises the actual Readable Studio app through the existing harness.

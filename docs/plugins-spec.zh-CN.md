# Readable Studio 插件与 Marketplace 规范（v1）

> **一句话总结：** Readable Studio 插件把可移植的 `SKILL.md` 能力包装成 marketplace 可发现、一键可用的设计工作流，同时保留对现有 agent skill 生态和 headless CLI 使用方式的兼容性。
...
**Plugin（插件）** 是 Readable Studio 的分发单元。[Skill](skills-protocol.md) 描述的是 agent 可以执行的一项能力，而 Plugin 是围绕这项能力形成的可发布包：一个或多个 skills、可选 design system 引用、可选 craft 规则、可选 Claude-plugin 资产、预览、use-case query、资产文件夹，以及一个用于驱动 Readable Studio marketplace 表面的轻量机器可读 sidecar。插件始终以可移植的 `SKILL.md` 为锚点，因此可以不经修改地发布到现有 agent skill 生态。

> **兼容性承诺（扩展 [`skills-protocol.md`](skills-protocol.md)）：** 任何包含 `SKILL.md` 的插件文件夹，都可以作为普通 agent skill 在 Claude Code、Cursor、Codex、Gemini CLI、OpenClaw、Hermes 等工具中运行。添加 `readable-studio.json` 只是纯增量能力：它会解锁 Readable Studio 的 marketplace 卡片、预览、一键「使用」流程、类型化 context-chip strip，但不会改变底层 skill 的运行方式。**一个 repo，两种消费模式。**

## 给读者的全局梳理

这是一套 **agent 时代的插件系统**，不是 Figma 时代的 UI extension system。插件不会直接挂载到 canvas 里，不拥有本地面板生命周期，也不通过一套专用 `postMessage` / RPC 协议和宿主应用通信。插件本质上是一组可打包的意图与上下文：用户、UI、CLI 或另一个 agent 选中它之后，Readable Studio 把它解析成 query、context chips、assets、design-system 引用、MCP/tool capabilities 和 run metadata；之后由选定的 code agent 通过 Readable Studio 统一的 project/run pipeline 消费这些信息。

最短心智模型：

1. **插件作者发布可移植能力。** `SKILL.md` 仍然是可执行的 agent contract；`readable-studio.json` 增加 Readable Studio marketplace 元数据、输入字段、默认值、预览和上下文 wiring。
2. **用户或 agent 选择一个工作流。** 选择入口可以是 marketplace、首页输入框、已有 project chat、CLI，或者 CI。
3. **Readable Studio apply 插件，但插件不是 UI 进程。** Apply 返回 hydrated brief、类型化 context chips、assets 和 capability requirements；它不会启动隐藏的插件 runtime。
...
| 问题 | Figma 时代插件假设 | Readable Studio v1 的答案 |
| --- | --- | --- |
| 谁消费插件？ | 宿主 UI runtime。 | code agent 通过 Readable Studio project/run pipeline 消费。 |
| 插件需要 live UI lifecycle 吗？ | 通常需要：挂载 panel、监听消息、修改 document。 | 不需要。插件是静态文件加 manifest；活跃进程是 agent run。 |
| 是否需要插件到 app 的 RPC 协议？ | 经常需要。 | 不是 primary contract。Readable Studio 内部用 HTTP，agent 用 CLI，合适的场景用 MCP，run events 用 SSE/ND-JSON。 |
| 「Use plugin」会发生什么？ | 打开或运行一个 UI extension。 | Hydrate query、context chips、assets、inputs 和 capability gates，然后创建或继续一次 agent run。 |
| 什么会被持久化？ | 宿主 document mutations。 | Readable Studio project metadata、artifacts、conversation/run history 和 plugin provenance。 |
...
sequenceDiagram
  ...
  participant S as Readable Studio 表面<br/>(Web, Desktop, CLI, CI)
  participant D as Readable Studio Daemon
...
Readable Studio 的核心不是「一次 prompt 一次输出」，而是 **long-running design agent 任务**：单个 run 通常经过 discovery → 方向选择 → 生成 → critique → 二次调优等多步，可能跨越数十分钟到数小时。插件的本质是把这条长程任务**切成可发布的单元**，让用户、UI、CLI 或另一个 agent 一键启动。

围绕这一点，spec 把已有的「一方 atoms」从扁平的 capability 列表升级为**可被插件组装的原子管线**：

- **Atom（§10）**：Readable Studio daemon 与 first-party tools 暴露的具名能力（discovery-question-form、direction-picker、todo-write、file-read/write、research-search、critique-theater 等），以及 report、prototype、deck、template 等交付物类型。
- **Pipeline（§5 / §10.1）**：插件通过 `readable.pipeline` 把若干 atoms 组装成有序 stages；spec 默认提供一条「discovery → plan → generate → critique」的 reference pipeline，插件可以增删、重排或循环其中任何一步。
...
**当前实现澄清：** `discovery -> plan -> generate -> critique` 是 reference pipeline 形态，不是一套写死的 wizard。插件 snapshot 可以携带 `readable.pipeline.stages[].atoms[]`；daemon 解析 snapshot 后，把 active plugin block 与 active stage atom blocks 注入 system prompt，同时发出 stage events，让 agent 按 pipeline 推进。如果用户没有显式选择插件，Readable Studio 也**不是**启动一个通用裸 agent：Readable Studio 基础 designer prompt 与 discovery 规则始终存在。产品入口会在此基础上绑定合理默认值：Home 自由输入走内置隐藏的 `readable-default` scenario，按类型创建新 project 时走对应 project kind 的 bundled scenario。`readable-default` 是 router / task shaper；它的职责是把请求导回正常设计 pipeline，不应被理解成一个独立的「美化生成器」。

### 四类产品场景

| 场景 (`readable.taskKind`) | 用户起点 | 插件贡献 | 典型 atom 序列 |
| --- | --- | --- | --- |
...
| `tune-collab` | 已有 Readable Studio project 与 artifact | 在已有 artifact 上做 critique-tune、品牌切换、A/B、stakeholder review | direction-picker → patch-edit → critique → handoff |
...
4. [插件文件夹形态](#4-插件文件夹形态)
5. [`readable-studio.json` schema](#5-readable-studiojson--schema-v1)
6. [`readable-studio-marketplace.json` schema](#6-readable-studio-marketplacejson--联邦目录)
7. [发现与安装](#7-发现与安装)
8. [Apply pipeline](#8-apply-pipeline)
9. [信任与能力](#9-信任与能力)
10. [一方 atoms](#10-一方-atoms--readable-studio-的原子能力)
11. [架构：现有 repo 要改什么](#11-架构现有-repo-要改什么)
12. [CLI 表面](#12-cli-表面)
13. [公网 Web 表面](#13-公网-web-表面-readable-studioaimarketplace)
...
19. [为什么这是 Readable Studio 的重要一步](#19-为什么这是-readable-studio-的重要一步)
...
23. [自举：把 Readable Studio 自己的硬流程做成一方 plugin](#23-自举把-readable-自己的硬流程做成一方-plugin)
...
- 写作阶段：草稿，等待评审。
- 规划阶段锁定的默认选择（评审时可覆盖）：
  - **兼容性 = wrap-then-extend。** 现有 `SKILL.md` 与 `.claude-plugin/plugin.json` repo 原样可运行；`readable-studio.json` 是增量 sidecar。
...
Readable Studio 变成一套 **server + CLI + atomic core engine + plugin/marketplace system**。产品表面发生反转：不再是「点一个按钮，填一个表单」，而是用户打开 marketplace，点击某个插件，输入框自动填入 query，并在上方注入类型化 context chips。相同的插件文件夹也同时是 Claude Code、Cursor、Codex、Gemini CLI、OpenClaw、Hermes 可消费的 agent skill，并且可以作为独立 GitHub repo 发布到：
...
不同目录的收录格式不同，但它们都索引 `SKILL.md` 形态的文件夹。只要保持 `SKILL.md` 作为 canonical，`readable-studio.json` 作为严格 sidecar，一个 repo 就可以不做目标目录专用改写而进入所有生态目录。

同一愿景的第二条轴线：**CLI 是 Readable Studio 面向 agent 的 canonical API。** 代码 agent（Claude Code、Cursor、Codex、OpenClaw、Hermes、企业内部 orchestrator）通过 shell 调用 `readable …` 驱动 Readable Studio，而不是直接请求 `/api/*`。CLI 用稳定的子命令 contract 包装所有 server 能力：project 创建、conversation/run 生命周期、plugin apply、project 文件系统操作、design library introspection、daemon control。HTTP server 是 desktop UI 与 CLI 自身的实现细节；agent 如果直接访问 HTTP，就绕过了 contract。

第三条轴线来自第二条：**Readable Studio 可以完全 headless 运行；UI 是效率层，而不是运行时依赖。** 用户只有 Claude Code（或 Cursor、Codex、Gemini CLI）和已安装的 `readable`，也能浏览 marketplace、安装插件、创建 project、拉起任务、消费产物，全流程不需要启动 desktop app。Readable Studio desktop UI 的价值类似 Cursor IDE 相对于 `cursor-agent` CLI：更快发现、实时 artifact preview、chat/canvas 并排、marketplace 浏览、direction-picker GUI、critique-theater 面板。这些都是同一批 primitives 之上的体验增强。每个 UI 功能都必须先能表达为 CLI 子命令或 streaming event；UI 消费这些 primitives 并添加呈现层。这个解耦由架构规则强制（§11.7）。
...
第五条轴线是与 agent 共演进的产品形态：**UI 由 agent 请求，但由产品内的受控组件渲染 (Generative UI)，而不是任意 agent-authored frontend code。** 长程 design agent 在跑 pipeline 的过程中，会随时需要向用户索取信息（例：figma OAuth、确认目标受众）、寻求授权（例：批准一次 high-cost media 生成）、收敛方向（例：从 3 个 critique 选项里选一个）、补内容（例：缺失的品牌资产）；这些 UI **不是**预制的 marketplace chip strip，而是 plugin 在 manifest 中**声明**、agent 在 run 中**触发**、daemon 用 Readable Studio 原生事件**发布**给前端 / CLI 渲染，并通过 AG-UI adapter 供外部 client 消费的 surface（详见 §10.3）。Readable Studio v1 提供 4 类内置 surface（`form` / `choice` / `confirmation` / `oauth-prompt`）作为最小集；plugin 自带 React 组件必须经过 `genui:custom-component` capability gate 与 sandbox。对应这条轴线，project 表多记录一组 GenUI surface 的 **persisted state**：用户做过一次的授权与确认在同一 project 的多轮对话、多次 run 之间复用，不会被反复打扰；这是「插件 = 长程任务封装」与「project = 长期工作产物」的自然落点。

## 2. 目标与非目标

**目标**

1. 每个可运行、可分发的 Readable Studio 插件都是合法 agent skill（以 `SKILL.md` 或 `.claude-plugin/plugin.json` 为锚点）。不 fork skill spec。
2. 一个普通 skill 或 claude-plugin repo，只要添加可选 `readable-studio.json` sidecar，就成为 Readable Studio 插件；不改名、不改 body。
3. 支持四类安装源：本地文件夹、GitHub repo（可带 ref/subpath）、任意 HTTPS archive，以及联邦 `readable-studio-marketplace.json` index。
...
6. Readable Studio core engine、atomic capabilities、plugin runtime 全部可以通过 CLI 访问，因此任何 code agent 都能 headless 地驱动 Readable Studio。
7. **插件即长程任务封装**：每个插件覆盖四类产品场景之一（new-generation / code-migration / figma-migration / tune-collab），通过 `readable.pipeline` 把 Readable Studio 一方 atoms 组装成有序 stages + 可选 devloop（§10）。
...
10. **Generative UI 是 plugin 一等输出**：插件在 manifest 中声明 `readable.genui.surfaces[]`（§10.3），agent 运行时通过 Readable Studio 受控事件流发布 form / choice / confirmation / oauth-prompt 请求，产品 renderer 负责视觉样式；用户的回应按 `persist` 等级（run / conversation / project）落到 project metadata，被同 project 后续多轮对话、多次 run 复用。外部 AG-UI client 通过 adapter 消费同一条 run，而不是替换 Readable Studio 内部 renderer。

**非目标（v1）**

- 替代 SKILL.md / claude-plugin spec：Readable Studio 永不 fork。
- 托管插件二进制：Readable Studio 指向 GitHub / CDN URL；存储由发布者负责。
- 签名/PKI 生态：能力 gating 依赖用户授权，而不是签名。
- 一个由 Readable Studio SaaS 代表用户运行 agent 的 web-hosted marketplace：v1 只做 local-first / self-hostable。

## 3. 兼容性矩阵：什么样的文件夹对哪些系统是合法插件

| 存在的文件 | Readable Studio 可安装 | Claude Code / Cursor / Codex / Gemini CLI | OpenClaw / Hermes | awesome-agent-skills | clawhub | skills.sh |
| --- | --- | --- | --- | --- | --- | --- |
| 仅 `SKILL.md` | yes | yes | yes | yes | yes | yes |
| 仅 `.claude-plugin/plugin.json` | yes | yes (claude) | partial | listable | listable | listable |
| 仅 `readable-studio.json` | metadata-only | no | no | no | no | no |
...
结论：**`SKILL.md` 是最低共同分母**。任何推荐用于分发的插件都应该包含 `SKILL.md`，这样它可以进入所有主流目录；再添加 `readable-studio.json` 来获得 Readable Studio 的产品表面。

仅包含 `readable-studio.json` 的文件夹在 v1 中不是可运行插件，而是 **metadata-only preset**：Readable Studio 可以读取它来展示市场卡片、聚合远端引用或作为未来 install stub，但它不能触发 agent run，也不能进入跨 agent catalog。`readable plugin doctor` 必须把这种形态标记为 `metadata-only`，并提示作者补充 `SKILL.md` 或 `.claude-plugin/plugin.json` 才能发布为 runnable plugin。
...
│   └── plugin.json
├── readable-studio.json                  # optional sidecar — unlocks Readable Studio product surface
├── README.md                         # standard catalog readme
├── preview/                          # Readable Studio preview assets
│   ├── index.html
│   ├── poster.png
│   └── demo.mp4
...
- `SKILL.md` body 不承载 Readable Studio 专属元数据；它保持干净、可移植。
- `readable-studio.json` 只**指向** SKILL.md / DESIGN.md / craft 文件；永不复制它们的正文。
- 当前 SKILL.md frontmatter 上已有的 Readable Studio 专属 `readable:` namespace（已在 [`skills-protocol.md`](skills-protocol.md) 中描述，并在 [`skills/blog-post/SKILL.md`](../skills/blog-post/SKILL.md) 中使用）继续作为没有 `readable-studio.json` 的插件的 fallback。我们不废弃它，只在其上叠加。
- v1 的 runnable plugin 必须至少包含 `SKILL.md` 或 `.claude-plugin/plugin.json` 之一。`readable-studio.json` 本身不定义 agent 行为，只定义 Readable Studio 如何展示、解析和应用这些行为。
...
```json
{
  ...
  "author":   { "name": "Readable Studio", "url": "https://github.com/sanghyunna/readable-studio" },
  "license":  "MIT",
  "homepage": "https://github.com/readable-studio/plugins/make-a-deck",
  "icon":     "./icon.svg",
  ...
  "readable": {
    ...
    "platform": "desktop",
    "scenario": "marketing",
    "engineRequirements": { "readable": ">=0.4.0" },
...
- `compat.*`：指向继承格式文件的相对路径。loader 会把它们的内容合并进 [`composeSystemPrompt()`](../apps/daemon/src/prompts/system.ts) 组装出的 Readable Studio prompt stack。
- `specVersion`：解释此 manifest 时使用的 Readable Studio 插件规范版本。它独立于插件 `version`，并会冻结到 apply snapshot，便于 replay。
...
- `readable.kind`：registry 里的分类（`skill` / `scenario` / `atom` / `bundle`）。
- `readable.taskKind`：四类产品场景之一（`new-generation` / `code-migration` / `figma-migration` / `tune-collab`，§1「四类产品场景」）。决定 marketplace filter、初始 inputs 模板、推荐 pipeline 起点。
- `readable.preview`：驱动 marketplace 卡片和详情页。`entry` 通过 daemon 以 sandboxed 方式服务（扩展现有 `/api/skills/:id/example` plumbing）。
- `readable.useCase.query`：一键使用时进入 brief 字段的精确文本。它可以是兼容旧 manifest 的字符串，也可以是按 BCP-47 风格 locale key 组织的文本映射（例如 `{ "en": "...", "zh-CN": "..." }`）。apply 时会依次尝试请求的 locale、基础语言、`en`，最后回退到第一个可用值。`{{var}}` placeholder 绑定到 `readable.inputs`。
- `readable.context.*`：用于填充输入框上方 `ContextChipStrip` 的类型化 chips。每一项都会编译成一个 `ContextItem`（§5.2）。
- `readable.context.atoms`：**无序集合**——声明插件需要的 atoms。daemon 仅以默认顺序使用它们；用于不需要自定义流程的简单插件。
- `readable.pipeline`：**有序管线**——插件作者显式编排 atoms 的 stages、循环、终止条件（§10.1）。当 `readable.pipeline` 与 `readable.context.atoms` 同时出现时，pipeline 优先；context.atoms 仅作为 chip strip 展示。
- `readable.genui.surfaces[]`：**Generative UI 声明**——agent 在 run 中可能触发的 surface 集合（§10.3）。每条 entry 的 `kind` 是 v1 内置的 `form` / `choice` / `confirmation` / `oauth-prompt` 之一；`persist` 决定回答的留存层级（`run` / `conversation` / `project`）；`trigger` 把 surface 绑定到具体 stage / atom，避免 agent 滥用；`schema` 是 JSON Schema，daemon 据此渲染默认表单并校验回答。**未在 manifest 中声明的 surface kind 不可在 run 中触发**——doctor 校验保证 plugin 不会动态产生未知 UI。

- `readable.inputs`：详情页与 inline composer 中展示的表单字段；其值会 template 到 `useCase.query` 以及任何 string-valued context entry。
- `readable.capabilities`：声明式能力；如果一个 `restricted` 插件省略此字段，默认是 `['prompt:inject']`。
...
- 声明 `mcp` 且 MCP command 不是 Readable Studio 内置 stdio tool 时，隐含需要 `subprocess`。
...
- provider credentials 不是 v1 capability；插件不能直接声明访问 `ANTHROPIC_API_KEY`、media provider keys 或 MCP server secrets。需要 credentials 的能力必须通过 Readable Studio 自己的 first-party atom / tool 间接调用。

### 5.4 `SKILL.md` frontmatter 到 `PluginManifest` 的映射

当插件没有 `readable-studio.json`，但 `SKILL.md` 已经包含 [`skills-protocol.md`](skills-protocol.md) 中定义的 `readable:` frontmatter 时，`adapters/agent-skill.ts` 会合成一个最小 `PluginManifest`。映射规则必须稳定，避免旧 skill 与新 plugin schema 出现两套语义：

| `SKILL.md` 字段 | Plugin manifest 字段 | 规则 |
| --- | --- | --- |
| `name` | `name`、fallback `title` | `title` 缺失时从 `name` humanize |
| `description` | `description` | 原样保留 |
| `readable.mode` | `readable.mode` | 原样保留；缺失时沿用 `skills-protocol.md` 的 keyword inference |
| `readable.preview` | `readable.preview` | `type` / `entry` / `reload` 中仅 `type`、`entry` 进入 v1 manifest；`reload` 保留为 adapter metadata，不进入公共 contract |
| `readable.design_system.requires` | `readable.context.designSystem` | `true` 表示运行时使用 active project design system；`sections` 作为 prompt-pruning hint 保留在 resolved context 中 |
| `readable.craft.requires` | `readable.context.craft` | slug 数组原样映射 |
| `readable.inputs` | `readable.inputs` | `string` → `string`，`integer` → `number`，`enum` → `select`，`values` → `options`；`min` / `max` 保留为 future metadata，v1 UI 可以忽略但不得丢失 |
| `readable.parameters` | adapter metadata | v1 plugin apply 不渲染 live sliders；字段保留给 Phase 4，不进入 `ApplyResult.inputs` |
| `readable.outputs` | `projectMetadata` hints | 用于 artifact bookkeeping 和 preview default，不作为用户可编辑 input |
| `readable.capabilities_required` | `readable.capabilities` | 只映射能表达的能力；未知 capability 保留为 `compatWarnings[]`，`readable plugin doctor` 必须提示 |

如果 `readable-studio.json` 与 `SKILL.md` frontmatter 同时存在，`readable-studio.json` 优先，但 loader 必须保留 adapter 产生的 warnings。这样作者可以渐进迁移：先让旧 skill 原样可用，再逐步增加 Readable Studio marketplace 信息。

## 6. `readable-studio-marketplace.json` — 联邦目录
...
```json
{
  ...
  "version": "1.0.0",
  "owner":    { "name": "Readable Studio", "url": "https://github.com/sanghyunna/readable-studio" },
  "metadata": { "description": "First-party plugins", "version": "1.0.0" },
  "plugins": [
    { "name": "make-a-deck", "version": "1.0.0", "source": "github:readable-studio/plugins/make-a-deck", "tags": ["deck"] },
    { "name": "tweet-card",  "version": "1.0.0", "source": "https://files.../tweet-card-1.0.0.tgz",  "tags": ["marketing"] }
  ]
}
```

Marketplace 顶层 `version` 是 catalog snapshot 版本；每个 `plugins[]` entry 也声明被列入的插件版本。Installer 抓取后仍会校验目标文件夹自己的 `readable-studio.json`，但 registry search、审计日志和 marketplace refresh events 可以在安装前就理解 catalog 与插件版本。
...
| Priority | 路径 | 资源形态 | 来源 |
| --- | --- | --- | --- |
| 1 | `<projectCwd>/.readable-studio/plugins/<id>/` | plugin bundle | 新增，与用户代码一起提交；必须显式安装到 project |
...
| 4 | `~/.readable-studio/skills/<id>/` | legacy `SKILL.md` | Readable Studio canonical skill install path；可 symlink 到其它 agent |
...
- **Inline apply**（shallow，核心产品表面）：Home 输入框与现有 project 内的输入框（ChatComposer）都在下方渲染一个 **inline plugins rail**。点击 rail 中的插件卡片会在原地应用插件，不跳转、不丢上下文。brief input 预填，输入框上方 chip strip 填充，插件的 `readable.inputs` 表单空项显示在 input 与 Send 按钮之间。用户填几个空、微调 brief、点击 Send。这个自然语言 + 填空的交互同时驱动 project 创建和已有 project 内的任务。
...
sequenceDiagram
  ...
  D->>FS: resolve readable.context refs<br/>(skills, DS, craft, atoms, assets, mcp)
  D-->>W: ApplyResult { query, contextItems, assets, mcpServers, projectMetadata }
  W->>H: navigate, hydrate NewProjectPanel
  Note over H: brief = useCase.query<br/>chip strip = contextItems<br/>inputs form = readable.inputs<br/>attachments = asset refs
...
sequenceDiagram
  ...
  Note over C: brief = useCase.query<br/>chip strip = contextItems<br/>inputs form = readable.inputs (visible blanks)<br/>attachments = asset refs
...
- `readable.pipeline` / `readable.context.*` 中的 ref 可能指向移动的 default branch。
...
### 8.3 Inline `readable.inputs` form

当 applied plugin 声明 `readable.inputs` 时，composer 在 brief textarea 与 Send button 之间渲染 `PluginInputsForm`。行为如下：
...
`restricted` 插件永远不能进入 P3/P4，除非用户授权该能力——通过 `readable plugin trust <id>` 或详情页上的「Grant capabilities」。默认 trusted 的来源只有两类：repo bundled first-party 插件，以及 Readable Studio 官方 marketplace。用户添加的第三方 marketplace 只提供 discovery；其中插件默认仍是 `restricted`，除非该 marketplace 本身被显式 trust，或某个插件按 id + version + capability 单独授权。
...
当插件升级、resolved ref 变化或 source marketplace 变化时，高危能力（`mcp`、`subprocess`、`bash`、`network`）需要重新确认。
...
`bash` / `subprocess` / `network` 等 elevated capabilities **绝不**支持 `--grant-caps all` shorthand 单独覆盖：CLI 强制要求显式列出每一项，避免脚本误授权。

### 9.2 Preview sandbox

插件 preview 可能来自未信任 GitHub / archive，因此不能与 Readable Studio app 共用同一执行权限。`readable.preview.entry` 的 HTML 预览必须满足以下约束：
...
把 [`apps/daemon/src/prompts/system.ts`](../apps/daemon/src/prompts/system.ts) 与 daemon-backed bash tools 里已有的能力提升为**具名、可声明、可被插件组装**的 atoms。Atom 不是孤立的 capability tag——它是 Readable Studio 长程 design agent pipeline 的一个节点，可以被插件拼接成有序 stages，并通过 devloop 在 stage 内循环。v1 仍然是**声明式**：daemon 已经知道如何为每个 atom 发出 prompt fragment 与必要的 tool gating，插件只是声明 pipeline 拓扑。
...
### 10.1 `readable.pipeline`：插件组装的有序原子管线
...
- 当 `readable.pipeline` 缺省时，daemon 按 `readable.taskKind` 选择 reference pipeline（§1「四类产品场景」中各场景列出的典型序列）。
...
- **iterations 上限**：daemon 强制 `iterations <= 10`（可通过 `READABLE_MAX_DEVLOOP_ITERATIONS` 调整），防止 plugin bug 导致无限循环烧 provider quota。
...
| `confirmation` | 二选一确认（继续 / 取消、批准 / 拒绝） | 行内 Yes/No 按钮 | 任何高代价 atom，例如 `subprocess` 类 hook | `run` |
| `oauth-prompt` | 拉起第三方 OAuth（Figma、Notion、Slack 等） | 弹窗 + 引导文案 | MCP 的鉴权需要 | `project` |
...
export interface GenUISurfaceSpec {
  ...
  capabilitiesRequired?: string[];             // 例：oauth-prompt with route='mcp' 需要 'mcp' capability
  ...
  oauth?: {
    route: 'mcp';                              // v1 仅 'mcp'；'plugin' 留给 Phase 4
...
| 维度 | v1（Readable Studio 原生） | AG-UI adapter / 外部兼容 |
| --- | --- | --- |
| Wire format | Readable Studio 自有 SSE / ND-JSON `PersistedAgentEvent` | 同时输出 AG-UI canonical events（包括 `agent.message`、`tool_call`、`state_update`、`ui.surface_requested`、`ui.surface_responded`） |
| Surface kinds | 4 类内置 + plugin 在 manifest 中声明 | Readable Studio 内置 surface 仍是产品 source of truth；plugin React 组件路径必须经过 `genui:custom-component` gate 与 sandbox |
| Shared state | `genui_surfaces` 表 + `genui_state_synced` 事件 | 把 Readable Studio 持久化状态映射到 AG-UI 的 `state` channel，供外部消费者使用 |
| Frontend SDK 兼容 | Readable Studio desktop / web 自带 renderer | `@readable-studio/agui-adapter` 让 CopilotKit / 其他 AG-UI client 直接消费 Readable Studio run |

Adapter 是互操作表面，不是内部 UI 的 source of truth。除非另有外部 embed / demo / client 明确需要，否则 Readable Studio 不应把 CopilotKit 加成主产品依赖。v1 plugin **不需要改**就可以在 AG-UI ecosystem 内被消费，因为 adapter 是 Readable Studio 自有事件的一层投影。
...
- `parsers/manifest.ts`：读取 `readable-studio.json` → `PluginManifest`（Zod-validated）。
- `adapters/agent-skill.ts`：读取 `SKILL.md` → 基于 [`skills-protocol.md`](skills-protocol.md) 里的 `readable:` frontmatter 合成 `PluginManifest`。
- `adapters/claude-plugin.ts`：读取 `.claude-plugin/plugin.json` → 合成 `PluginManifest`。
- `merge.ts`：合并 sidecar + adapters，`readable-studio.json` 优先；foreign content 落到 `compat.*`。
- `resolve.ts`：针对 registry 解析 `readable.context.*` refs → `ResolvedContext`。
...
| 新 `apps/daemon/src/plugins/pipeline.ts` | 解析 `readable.pipeline`（含 `until` expression 求值器）、调度 stages、控制 §10.2 devloop（含 `READABLE_MAX_DEVLOOP_ITERATIONS` 上限与 break 信号）。 |
| 新 `apps/daemon/src/genui/{registry,events,store}.ts` | §10.3 GenUI surface 注册（来自 `readable.genui.surfaces[]`）、`genui_surface_*` event 发布、cross-conversation persisted state 读写、AG-UI–inspired event union 序列化。 |
| [`apps/daemon/src/prompts/system.ts`](../apps/daemon/src/prompts/system.ts) `composeSystemPrompt()` | 组装 Readable Studio 基础 designer/discovery prompt、可选 design system/craft/skill blocks、snapshot 派生的 `renderPluginBlock(snapshot)`，以及来自 `renderActiveStageBlock(stageId, bodies)` 的 active-stage atom blocks。现有层级顺序保持有意设计；plugin-block renderer 已在 contracts 中，但 plugin-driven fallback mode 仍按 §11.8 拒绝。 |
...
-- runs / conversations / projects 上加可选指针；Readable Studio 主键不变，向后兼容
...
- 当 plugin 声明 output hints 时，daemon 写入 `artifactKind`、`renderKind` 与 `handoffKind`；否则根据 `readable.mode`、`readable.preview.type` 和实际产出文件推断最保守的值。未知 reader 即使暂时不用这些字段，也必须原样 preserve。
...
| 新 `apps/web/src/components/PluginInputsForm.tsx` | 把 `readable.inputs` 渲染成 input 与 Send 之间的 inline form。required fields gate Send。inputs 变化时，`useCase.query` 和 chip labels 里的 `{{var}}` 实时 re-render。 |
...
Readable Studio 运行在三种 operating modes，它们共享**同一个** daemon、**同一个** CLI、**同一个** plugin runtime。差异只是呈现方式。这与 Cursor 的形态一致：`cursor-agent`（CLI）自身足够运行；IDE 是增强层。
...
- 用户只有 **Claude Code**（或任意 code agent）加 `npm i -g @readable-studio/cli`，再启动一个 headless daemon，就能完成 install plugin → create project → run → consume artifacts 全流程。不需要 Readable Studio desktop。
- Readable Studio desktop UI 安装相同 daemon 与相同 CLI；它只是加了一个窗口。用户之后安装 desktop 时，会看到 headless 流程创建的同一批 projects、plugins、history。不存在「headless project format」与「desktop project format」之分。都是同一个 `.readable-studio/projects/<id>/`、同一个 SQLite db。
...
- 外部产品可以通过启动 headless daemon 并 shell out 嵌入 Readable Studio：`readable` 是 public surface，internals 可以自由演进。
...
Readable Studio 当前有**两份** `composeSystemPrompt()` 实现：
...
由此，§14.2 列出的「skill-only consumption / headless Readable Studio / full Readable Studio」三种消费模式中，只有后两者拥有 plugin context；这与 §1 中「插件是 long-task design agent 的封装」一致——长程任务必须有 daemon 或 daemon 等价物（headless）才能被组装。

这也回答「没有选择插件」路径：没有 applied plugin 的 run 仍然会收到 Readable Studio 基础 prompt stack（`DISCOVERY_AND_PHILOSOPHY`、official designer instructions、project metadata、存在时的 active design system/craft，以及入口选择的默认 scenario routing）。Plugin context 是增量项。选择插件会追加 snapshot 派生的 `## Active plugin`、`## Plugin inputs` 与 active-stage atom blocks；它不是把一个非设计 agent 从零变成设计 agent 的唯一来源。

## 12. CLI 表面

CLI（`readable …`）是 **Readable Studio 面向 agent 的 canonical API**。Plugin verbs 只是其中一部分；CLI 的其它部分包装 daemon core capabilities：projects、conversations、runs、file operations、design library introspection、daemon control，因此任何 code agent 都能通过 shell calls 端到端驱动 Readable Studio。这是「通过 CLI 用自然语言创建 project + task」的路径：code agent 读取用户请求，然后发出一串 `readable …` 调用，而不是直接说 HTTP。
...
daemon 当前已经拥有 `.readable-studio/projects/<id>/` 下的 project filesystems（或者 imported folder 的 `metadata.baseDir`）。这些命令以 project 为 scope：agent 不需要知道 project 在磁盘哪里。
...
当目标是 Readable Studio-managed projects 时，code agent 通常应该用 `readable files read` / `readable files write`，而不是 native file ops，因为 daemon 负责 artifact bookkeeping（[`packages/contracts/src/api/registry.ts`](../packages/contracts/src/api/registry.ts) 中的 `ArtifactManifest.sourceSkillId` 等）。
...
readable plugin export    <projectId> --as readable|claude-plugin|agent-skill --out <dir>
...
一个通过 CLI 驱动 Readable Studio 的 code agent 通常这样做：
...
## 13. 公网 Web 表面（readable-studio.ai/marketplace）

产品网站已经是 [readable-studio.ai](https://github.com/sanghyunna/readable-studio)。公网 marketplace 作为同一站点下的路径发布：`readable-studio.ai/marketplace`（canonical），并把 `readable-studio.ai/plugins` 作为 alias；不是单独域名。它是一个从官方 `readable-studio-marketplace.json` index 渲染出的 static-rendered catalog，插件详情页由每个 listed repo 内的同一份 `readable-studio.json` 支撑。视觉上它类似 [`skills.sh`](https://skills.sh/) 对 skills 的处理，但详情页会渲染 Readable Studio 专属 previews（`readable.preview.entry` HTML、sample outputs、use-case query、chip preview）。

这个站点与 in-app marketplace 共享一个 source of truth：

- 同一组 JSON Schemas（`urn:readable-studio:schema:plugin-manifest:v1`、`urn:readable-studio:schema:plugin-marketplace:v1`）。
- 同一套联邦 listing format（`readable-studio-marketplace.json`）。
- 同一份 plugin manifests（每个 repo 内的 `readable-studio.json`）。

两个消费表面，一个 substrate：

| Surface | Audience | Primary CTA |
| --- | --- | --- |
| In-app marketplace（`/marketplace`，§11.6） | 已登录 Readable Studio 用户 | “Use this plugin” → apply in place |
| Public marketplace（`readable-studio.ai/marketplace`） | 匿名访客、SEO、分享 | Deep-link `readable-studio://plugins/<id>?apply=1`（自动在 desktop app 中安装并 apply），以及 “Copy install command” |

Deep-link contract（Phase 4 deliverable，但在这里先锁定 schema 支持）：

- `readable-studio://plugins/<id>`：在 in-app marketplace 打开 plugin detail。
- `readable-studio://plugins/<id>?apply=1[&input.k=v...]`：如果缺失则安装，然后用 supplied inputs apply。
- `readable-studio://marketplace/add?url=<urlencoded>`：注册新的联邦 catalog。

desktop app 注册 `readable-studio://` URL scheme；点击 `readable-studio.ai/marketplace` 上的按钮时，如果 desktop 已安装则启动 desktop，否则 fallback 到「How to install Readable Studio」流程。
...
| `readable-studio-marketplace.json` | entry 指向 `github:owner/repo` | `readable-studio.json` 丰富 listing |

### 14.1 作者工具

- `readable plugin scaffold`：写入 starter folder，包含 `SKILL.md`（行业标准，带 `readable:` frontmatter 以向后兼容）和 `readable-studio.json`（Readable Studio enrichment，`compat.agentSkills` 指向 SKILL.md）。
...
任何通过 `SKILL.md` 消费文件夹的 code agent，都可以在没有 Readable Studio 的情况下运行。插件是一个 repo，有三种合法消费模式：

1. **Skill-only consumption（完全没有 Readable Studio）。** Cursor 用户执行 `npx skills add readable-studio/make-a-deck`。Cursor 读取 `SKILL.md` 并运行 workflow。没有 Readable Studio CLI、没有 Readable Studio daemon。插件的 marketplace polish（`readable-studio.json`）被忽略，Cursor 看到的是普通 skill。
2. **Headless Readable Studio（CLI + code agent，无 Readable Studio UI）。** 高阶用户继续使用自己偏好的 code agent：Claude Code、Cursor、Codex 等，但把 Readable Studio 作为 side service 加进来，获得 plugin context resolution、project bookkeeping、design library injection、artifact tracking。无 browser、无 electron。具体 pipeline 见 §14.3。
3. **Full Readable Studio（CLI + code agent + Readable Studio UI）。** 与 (2) 相同，但加上 desktop 或 web UI，用于 live preview、marketplace browsing、chat/canvas split-view 等。

插件作者只写一次 SKILL.md。三种模式都消费它。

### 14.3 具体 headless pipeline（Claude Code + `readable` CLI，无 Readable Studio UI）

这与 cursor-agent + scripts 对 Cursor 的意义相同：code agent 做思考，Readable Studio CLI 提供 project / plugin / artifact substrate。

```bash
# One-time setup: install the Readable Studio CLI as an npm global (publishable as @readable-studio/cli).
...
# Install the Readable Studio plugin you want to drive (or an upstream agent skill — both work).
readable plugin install github:readable-studio/plugins/make-a-deck
...
# Path B — drive Claude Code directly inside the project cwd; Readable Studio only provides
...
cd "$CWD"
# Readable Studio has already staged the merged SKILL.md / DESIGN.md / craft / atoms into
# .readable-studio-skills/ inside the cwd, exactly as the desktop run would.
claude code "Read .readable-studio-skills/ and produce the deliverables the active plugin describes."
...
- Readable Studio daemon 不需要渲染任何东西；它作为 project + plugin + artifact server 工作。
- 同一个 project 之后在 Readable Studio desktop UI 中打开时，会显示 headless run 产生的完整 conversation history、files、artifacts，因为 storage layer 只有一套（[`spec.md`](spec.md) §4.6：`.readable-studio/projects/<id>/` + SQLite）。

### 14.4 类比：Cursor vs `cursor-agent`，Readable Studio desktop vs `readable` CLI

心智模型：

| Layer | Cursor | Readable Studio |
| --- | --- | --- |
| Headless agent CLI | `cursor-agent`（驱动 agent loop） | `readable run start --agent claude --follow` + `readable plugin run` |
| Local services / db | Cursor 的 background indexing / state | Readable Studio daemon、SQLite、`.readable-studio/projects/<id>/` |
| GUI productivity layer | Cursor IDE | Readable Studio desktop / web UI（`apps/web` + `apps/desktop`） |
...
- `READABLE_DATA_DIR` 与 `READABLE_MEDIA_CONFIG_DIR` 控制本地 runtime storage。
...
- JSON Schemas：`docs/schemas/readable-studio.plugin.v1.json` 与 `readable-studio.marketplace.v1.json`。
...
- `pnpm --filter @readable-studio/plugin-runtime test`（parser fixtures：pure SKILL.md、pure claude plugin、metadata-only readable-studio.json、三者组合、SKILL frontmatter mapping）。
...
(d) Web API-fallback mode（Readable Studio daemon 关闭、浏览器直连 provider）下，UI 中 inline rail 渲染 plugin 卡片但点击「Use」时弹出 daemon-required 提示；daemon 启动后再点击恢复正常。
...
- `readable plugin export <projectId> --as readable|claude-plugin|agent-skill`：从已有 project 生成 publish-ready folder。
...
- **§10.3.5 AG-UI 完整对齐：**
  - 新 package `@readable-studio/agui-adapter`：把 Readable Studio 的 `PersistedAgentEvent` + `GenUIEvent` 双向映射到 AG-UI canonical events（`agent.message`、`tool_call`、`state_update`、`ui.surface_requested`、`ui.surface_responded`）。
  - daemon 增加可选 `/api/runs/:runId/agui` SSE 端点，输出 AG-UI canonical events，使 CopilotKit / 其他 AG-UI client 直接消费 Readable Studio run。
  - Plugin manifest 升级允许 `readable.genui.surfaces[].component`：相对路径指向 plugin 内 React 组件（capability gate `genui:custom-component`），由 desktop / web renderer 在 sandbox 中加载。
...
Readable Studio 通过 [`skills-protocol.md`](skills-protocol.md) 中现有 `readable:` frontmatter loader 把它当作插件读取。不需要 `readable-studio.json`：这个插件缺少 marketplace polish，但完全可运行。
...
### 17.2 Enriched plugin（可跨目录发布，完整 Readable Studio 产品表面）
...
`SKILL.md` 保持可移植：Cursor / Codex / OpenClaw 可直接读取。`readable-studio.json` 增加 preview、query、chip strip、capabilities。这个 repo 可以不经修改地列入 §14 的所有 catalog。
...
| Risk | Mitigation |
| --- | --- |
| Readable Studio plugin schema 与更广义 skill spec 发生 drift | `readable-studio.json` 仅为 sidecar；永不修改 SKILL.md。Package-scoped compatibility fixtures 覆盖 public skill shape。 |
...
| Atoms-as-plugins 范围大 | Entry slice 已落地：bundled atom SKILL.md bodies 与 `renderActiveStageBlock()` 已存在；Readable Studio 基础 designer/discovery prompt 仍留在 daemon code，等待 §23 剩余迁移完成。 |
| Project-local plugins 被提交到用户 repo | 仅发现 `<projectCwd>/.readable-studio/plugins/`；通过 `readable plugin install --project` opt-in。 |
...
| `readable.pipeline` devloop 无限循环烧 quota | `until` 必填且 syntax 受限；`READABLE_MAX_DEVLOOP_ITERATIONS` 上限（默认 10）；UI 与 CLI 提供「Stop refining」打断。 |
| `READABLE_HOST` / `READABLE_BIND_HOST` 命名漂移 | 使用现有 `READABLE_BIND_HOST`；不引入 `READABLE_HOST` 别名。 |
| 没有 API-token guard 的 non-loopback bind 可能公开泄露 API | Daemon 在无 `READABLE_API_TOKEN` 时拒绝 non-loopback bind；`/api/*` enforce bearer-token middleware。 |
...
| 恶意 plugin 通过 GenUI surface 钓鱼用户敏感信息 | `readable.genui.surfaces[]` schema 必须列入 manifest 并由 `readable plugin doctor` 校验；运行时拒绝未声明 surface kind / surface id；`oauth-prompt` 与 `confirmation` 的 issuer / capability 信息显示「来自 plugin <id>，由 marketplace <id> 验证」；restricted 插件触发 `oauth-prompt` 前还需要 `network` capability 显式 grant（§9）。 |
| AG-UI 协议生态可能演进，Readable Studio 自有 wire-format 与 AG-UI canonical 漂移 | Readable Studio-native `GenUIEvent` 仍是内部 source of truth；`@readable-studio/agui-adapter` 是外部投影层，因此 upstream 协议升级不绑死 daemon 或 web renderer release cadence。 |
| `genui_surfaces` 表 跨 conversation 复用导致用户「忘记自己授过权」 | UI 的 `GenUIInbox`、CLI 的 `readable ui list --project <id>` 必须列出所有 `persist=project` 的 resolved row 与 revoke 入口；hosted mode 提供 `READABLE_GENUI_PROJECT_TTL_DAYS` 让 operator 设置默认过期；revoke 操作写 audit 日志。 |
...
- **Non-local client 的 trust propagation**：当前 spec 已锁定 arbitrary GitHub / URL / local 插件默认 `restricted`，第三方 marketplace 也默认不传递 trust。还需确认 operator 是否允许通过 `READABLE_TRUSTED_PLUGINS` 直接 trust 单个插件，还是必须先 trust 它所属 marketplace。
...
- ~~**`AppliedPluginSnapshot` 留存策略**~~：**已按 PB2 解决。** 被 run / conversation / project 引用的 snapshot 永久 pin；未引用 snapshot 默认 30 天过期；`READABLE_SNAPSHOT_RETENTION_DAYS` 是 operator opt-in；`readable plugin snapshots prune` 保留为强制清理出口。
...
- **`readable.taskKind` 是否成为 marketplace 一等过滤维度**：现有 `kind` / `mode` / `scenario` 是否需要为新增的 `taskKind` 重排 UI filter？（默认：marketplace 顶部增加 `taskKind` tab；现有 filter 保留为二级。）
- ~~**`readable.genui.surfaces[].component` 是否进 v1**~~：**已作为 gated extension path 解决。** Manifest schema 接受该字段，`readable plugin doctor` 校验 `genui:custom-component` 与 path traversal；内置 `form` / `choice` / `confirmation` / `oauth-prompt` renderer 仍是主产品默认。
...
- ~~**AG-UI 协议引入时机**~~：**已解决。** `@readable-studio/agui-adapter` 与 `GET /api/runs/:runId/agui` 已作为可选互操作表面交付；Readable Studio-native GenUI 仍是内部 renderer，CopilotKit 不是主产品必需依赖。

## 19. 为什么这是 Readable Studio 的重要一步

- **继承供给。** `anthropics/skills`、`awesome-agent-skills`、`clawhub`、`skills.sh` 上的每个 public agent skill，只需一个可选 `readable-studio.json` 就能成为 Readable Studio 插件；反过来，每个 Readable Studio 插件也能不经修改发布到这些 catalog。
...
| `renderKind` | Readable Studio 应该如何 preview 或打开它 | `html`, `jsx`, `pptx`, `markdown`, `video`, `image`, `diff`, `repo` |
| `handoffKind` | 它对下游交付承诺到什么程度 | `design-only`, `implementation-plan`, `patch`, `deployable-app` |

§11.5.1 的 v1 `ArtifactManifest` 扩展已经预留这些 optional 字段。v1 producer 可以保守推断；v1 consumer 必须 preserve 未知值。这样后续阶段不需要依赖文件名 sniffing（`index.html`、`slides.json`、`diff.patch`），也不需要把 export 与 handoff 决策硬塞进 `readable.mode`。

### 20.2 Evaluator atoms

现有 `critique-theater` 与 devloop stage 是正确入口，但「critique」不能长期停留在纯 LLM 主观评价。后续 evaluator 能力应该作为 first-party atoms 接入 `readable.pipeline.stages[]`：
...
为方便阅读，这里把 Readable Studio 产品锁定的"四个核心 agent-native 设计问题"重申一次：
...
| 3 | 0→1 设计（prototype / deck / report / template） | `new-generation` | 是（§1 默认 reference pipeline） | **v1 已交付** —— 所需的 `discovery-question-form`、`direction-picker`、`todo-write`、`file-read` / `file-write`、`research-search`、`critique-theater` 全部 implemented | 可选：把 §20.2 的 `visual-diff` / `brand-consistency-check` 提前到 Phase 2，让 critique 有客观信号 |
| 4 | 设计 → 可交付业务代码 | `tune-collab`（handoff 侧） | 部分（§20.3 显式 post-v1） | **v1 不交付** —— v1 的 `handoffKind` 上限是 `'design-only' \| 'implementation-plan' \| 'patch'`；`'deployable-app'` 是 post-v1 | 要么完整落 §20.3，要么把 §14.3 的 Readable Studio ↔ 外部 code-agent 接力正式化为 v1 的"设计→生产代码"路径 |
...
2. **`readable.pipeline` + devloop 收敛**（§10.1，§10.2）。`repeat: true` + `until` 是场景 1、2、3 共用的收敛机制：场景 3 收敛在 critique 评分，场景 1 收敛在 token-map 保真度，场景 2 收敛在 `build-test` 通过。同一个 loop primitive 跑三个场景，差异仅在 `until` 信号。
...
5. **CLI-first headless 模式**（§11.7，§14.3）。这是 v1 阶段场景 4 的 fallback：Readable Studio 把 SKILL.md / DESIGN.md / craft / 生成 artifact 都 stage 到 project cwd，由 Cursor / Claude Code / Codex 在那个 cwd 内完成 patch。这条接力在 §14.3 已规定；§21.5 把它升格为 v1 的"设计→生产代码"明确契约。
...
- 在 Readable Studio 内部做的 repo-aware 多文件 diff 编排 + build evidence（场景 4 的 native 路径）。
...
- `token-map` atom —— 把抽出来的 tokens 映射到 active Readable Studio design system（`design-systems/<id>/DESIGN.md`）。spec 占了 id。
...
**v1 fallback（同时见 §21.5）：** 不在 Readable Studio 内部端到端跑这条，而是引导用户走 §14.3 的 headless pipeline —— Readable Studio 产出 `html-prototype` 或 `implementation-plan` artifact；Cursor / Claude Code 在用户自己的 repo cwd 里 apply patch 并跑 build/test。v1 不阻挡这条路径，只是不拥有 patch-application 这一步。
...
- 默认 reference pipeline `discovery → plan → generate → critique` 与典型 `new-generation` 流程一致；plugin 不需要声明 `readable.pipeline` 也能拿到一条工作 pipeline。
...
- "设计 → 生产代码" 在 v1 是一个**双产品接力**：Readable Studio 拥有 design substrate（SKILL.md / DESIGN.md / craft / 生成 artifact 都被 stage 进 project cwd，加上 `readable files` 管理的 artifact 簿记）；用户原本的 code agent（Cursor / Claude Code / Codex / Gemini CLI）在用户的 repo cwd 内拥有真实 repo patch。
...
- 加上在 Readable Studio 内部做的 repo-aware 多文件 patch 编排，并捕获 build evidence（这一步本身比 §20.3 还多迈一步）。
...
| 6 | 新增 "Phase 8 — 原生生产代码交付" | 在 Readable Studio 内部做 repo-aware 多文件 patch 编排；提供 native "review and apply" 表面；把 `handoffKind: 'deployable-app'` 从预留升格为实现 | 闭环场景 4 native 路径；依赖 Phase 7 |
...
### 21.5 Readable Studio ↔ code-agent 接力作为 v1 的生产代码路径

在 Phase 6、7、8 落地之前，生产代码体验对用户而言走的是 §14.3 headless pipeline。本节把这个模式升格为 v1 的契约，使外部 code-agent 集成不需要重读 §14.3 就可以依赖它。

契约锁定四点：

1. **Readable Studio 把 design substrate stage 进 project cwd。** 按 §14.3，daemon 把 SKILL.md / DESIGN.md / craft 写入 `.readable-studio-skills/`，把生成 artifact 通过 `readable files` 写入 project cwd。cwd 通过 `readable project info <id> --json | jq -r .cwd` 可发现。
2. **用户的 code agent 在该 cwd 或自己的 repo cwd 内操作。** Readable Studio 不在 IDE 里跑；它作为 daemon 与 IDE 并列。Cursor / Claude Code / Codex / Gemini CLI 是 patch-application 的表面。
3. **簿记留在 Readable Studio。** `ArtifactManifest`（§11.5.1）记录 `sourcePluginSnapshotId`、`sourceTaskKind: 'tune-collab'`（Phase 7 落地后还有 `'code-migration'`）、`handoffKind: 'patch'`；`readable files` 记录每一字节的 artifact。哪怕 patch 由 code agent 完成，Readable Studio 仍是 audit log 的拥有者。
4. **重新进入 Readable Studio 是 single-step。** 用户随时可以通过 inline rail（§8）或 `readable plugin apply ... --project <id>` 在同一个 project 上重新 apply 任意 plugin（或不同 plugin）。`parentArtifactId` 链（§11.5.1）跨越 Readable Studio ↔ code-agent 边界保留 lineage。

这就是用户层面问题"v1 阶段我能不能用这套 plugin 体系交付业务代码？"的回答：**可以，但交付方式是 Readable Studio substrate + 外部 code-agent 接力，不是 Readable Studio native 一键**。Phase 8（§21.4）才是 native 一键交付的路径。
...
§21 记录的是 v1 native 交付到哪。本节记录的是**开放性的另一半**：即便 v1 只为场景 3（`new-generation`）交付了完整的 native pipeline，第三方 plugin 作者今天就可以在 v1 substrate 上为场景 1、2、4 写出可运行的 plugin，**而不必等 §21.4 的 Phase 6 / 7 / 8 落地**。两半合起来 ——Readable Studio 一方交付（§21）+ substrate 给第三方的可达性（§22）—— 才能把"插件系统真的开放吗？"这个问题答成具体的"是"。
...
- **implementation**（实现）= 哪些 atom 已经作为 daemon 内置项可以一行写进 `readable.pipeline`（§10）。
...
| --- | --- | --- |
| 调一个 Readable Studio 没有的工具（Figma REST、AST 解析、SVG 转换…） | 在 `readable.context.mcp[]` 里捆绑一个 MCP server | §5 / §5.3（`mcp` + `subprocess` + `network`） |
| 在用户真实仓库上工作 | `readable project import <path>` 把 repo 装进 Readable Studio 的 project model；之后 `readable files` 与 agent 文件操作就地可用 | §12 / §11.7 / §14.3 |
...
| 多阶段 + 迭代收敛 | `readable.pipeline.stages[]` + `repeat: true` + `until` | §10.1 / §10.2 |
...
| 教 agent 一整套领域工作流 | `SKILL.md` body 注入 prompt + `readable.context.assets[]` 携带例子 | §11.3 `composeSystemPrompt()` |

总结成一条规则：**Readable Studio 一方 atom 缺位 → plugin 作者用 `MCP server + bash + SKILL.md` 三件套替代**。代价是 ergonomics（每个 plugin 各自重新发明命名和 prompt fragment），不是能力。
...
配套 SKILL.md 教 agent："收到 figma URL → 调 `figma-rest.get_file` → 遍历 node tree → 读 active design system 的 DESIGN.md → 在 cwd 里写 HTML，保留 figma 布局但用 DESIGN.md 的 token → 预览生成文件 → critique 循环"。**不需要等 Readable Studio 实现 `figma-extract` / `token-map`**；它们只是把 SKILL.md 的指令压成可复用的 prompt fragment。
...
#### 22.3.3 通过 Readable Studio ↔ code-agent 接力实现 设计→生产代码

plugin 作者在 v1 不应该试图把整段 handoff 都在 Readable Studio 内部完成。正确的形态是：
...
这就是 §21.5 的契约；plugin 作者写 SKILL.md 引导 agent 产出 handoff 形态的 artifact，到 Readable Studio 内部跑 patch 这一步就此打住。
...
2. **atom registry 是封闭的**。`readable.pipeline.stages[*].atoms[]` 必须命中 §10 catalog（implemented + reserved）。plugin 作者无法注册新 atom 名字。变通：把新能力封成 MCP tool，挂到最近的通用 atom（`file-write` / `todo-write` / `critique-theater`）下；daemon 发出的 `pipeline_stage_started` / `pipeline_stage_completed` 事件流就丢失这一步的粒度。
3. **核心 GenUI surface kind 在 v1 是封闭枚举**（§10.3.1）。产品自有 renderer 只有 `form` / `choice` / `confirmation` / `oauth-prompt` 四类内置 surface。plugin 作者想要分屏 diff review、画板涂改、3D 预览或其他高保真 UI，只能在可用时走独立的 gated `readable.genui.surfaces[].component` sandbox path；不能新增内置 surface kind，也不能覆盖主 renderer 的 styling contract。
...
## 23. 自举：把 Readable Studio 自己的硬流程做成一方 plugin

§22 确立了**第三方** plugin 在 v1 substrate 上的可扩展性。本节确立对称性质：**Readable Studio 自己写死的流程也可以被重新表达为运行在同一份 substrate 上的一方 plugin**，没有任何只有 Readable Studio 能用、第三方触不到的特权路径。plugin substrate 是真正的"地板"，Readable Studio-the-product 只是它上面的一种配置。
...
B 类的清单是**内核边界**：不是"难以 plugin 化"，而是"出于安全、持久化、运行时状态原因必须留在 daemon"。一个让 plugin 直接写 `applied_plugin_snapshots` 行或颁发 scoped tool token 的 plugin runtime 是坏掉的 runtime。
...
- plugin 声明的 MCP server、GenUI surface、`readable.pipeline.stages[]` 全部走的是和第三方 plugin 同一套 registry / resolver 路径。
...
- `plugins/_official/scenarios/**` 下的 bundled scenario plugins 已经携带默认 pipeline 形态，其中包括用于 Home 自由输入 routing / task shaping 的 `readable-default`。`packages/plugin-runtime/src/pipeline-fallback.ts` 会在 plugin 省略 `readable.pipeline` 时，通过这些 bundled scenarios 解析 applied pipeline。
- `@readable-studio/agui-adapter` 与 `/api/runs/:runId/agui` 提供外部 AG-UI event projection，同时不改变 Readable Studio 内部 GenUI renderer。

这就是 §22 成立的原因：substrate 已经在 plugin artifacts、snapshots、GenUI declarations、pipeline declarations、bundled scenarios 与第一条 atom-body injection path 上自举。剩余硬编码部分更窄，也更偏产品入口：Readable Studio 基础 designer/discovery prompt、部分 stage-entry 选择逻辑、Home curated scenario rail，以及 §22.4 中列出的封闭 signal / surface 词汇表。
...
#### 23.3.1 Patch 1 —— `readable.kind: 'atom'` 的正式契约

§5 的 `readable.kind` 枚举里有 `'atom'`，但从未定义 atom plugin 的形态。这条 patch：

- atom plugin 是带 `readable-studio.json`（`readable.kind: 'atom'`）+ `SKILL.md` 的目录。
- `SKILL.md` 正文就是该 atom 的 prompt fragment；当某个 stage 用 id 引用了该 atom，daemon assembler 把它注入。
- 可选的 `readable.context.mcp[]` 声明该 atom 用到的 MCP tool（如 `research-search`）。
- 可选的 `readable.atom.untilSignals[]` 声明该 atom 发出的命名信号变量，贡献到 §10.1 的 `until` 词汇表。这正是 patch 1 同时放开 §22.4 limit 1 的方式：每个 atom 自带信号（例如 `build-test` 声明 `build.passing` 和 `tests.passing`），`until` 求值器对照当前 stage 的 atoms 而非硬编码列表来查表。
- 可选的 `readable.atom.toolGating[]` 声明该 atom 期望可用的 agent 端工具（file-read/write/edit、bash）。
...
Bundled scenario plugins 与 pipeline fallback resolver 现在已经存在。剩余工作是移除任何仍然手写 default stage list 的产品入口，让它们改为选择 scenario plugin。目标仍然是：当 `readable.pipeline` 缺省时，daemon / product code 根据 `taskKind` 或显式入口 route 解析一个 bundled scenario plugin，然后使用该 scenario 的 `readable.pipeline`。

- `plugins/_official/scenarios/readable-new-generation/readable-studio.json`
- `plugins/_official/scenarios/readable-figma-migration/readable-studio.json`（Phase 6 后）
- `plugins/_official/scenarios/readable-code-migration/readable-studio.json`（Phase 7 后）
- `plugins/_official/scenarios/readable-tune-collab/readable-studio.json`

每个 plugin 只 ship 一个 `readable.pipeline` 加（可选）一些 scenario 默认的 `readable.genui.surfaces[]`。daemon 解析变成："没传 `readable.pipeline` + 有 `taskKind` → 查匹配 `taskKind` 的 bundled scenario plugin → 用它的 `readable.pipeline`"。

这条 patch 之后，**替换或 fork `readable-new-generation`** 是 ship 一个不同 Readable Studio 风味产品的标准做法。
...
1. 走 `<repo-root>/plugins/_official/**`，把每个 plugin 注册到 `installed_plugins` 中：`source_kind='bundled'`、`trust='bundled'`、capabilities = 该 plugin 声明的 `readable.capabilities`。
...
1. **审计 surface 收敛**。所有进入 agent 系统 prompt 的字节都从 plugin manifest + SKILL.md 可达。读 `plugins/_official/...` 就能完整回答"Readable Studio 今天在指挥 agent 干什么"，不需要 grep 代码。
2. **风味可替换**。企业部署、行业版本（地产 / 游戏 / 法律 / 财报）、伙伴集成可以替换自己的 `plugins/_official/scenarios/readable-<taskKind>` 而不需要 fork daemon。产品风味（discovery 提问风格、critique 维度、默认 pipeline 形态）从代码关切变成内容关切。
3. **一方工作走的是和第三方一样的路径**。Readable Studio 落 Phase 6 / 7 / 8（§21.4）的时候，那些场景就是住进 `plugins/_official/scenarios/` 的一方 plugin，跑的是任何社区 plugin 都在跑的 loader / resolver / snapshot / GenUI runtime。如果一方撞 ergonomics 墙，第三方撞得只会更狠 —— 这是最强的 dogfooding 压力来源。
...
| 章节 | 回答的问题 | 受众 |
| --- | --- | --- |
| §21 | Readable Studio 在 v1 里 native 交付了什么？哪些场景是 covered / partial / post-v1？ | 计划 §16 分阶段交付的 maintainer |
| §22 | 不等 Readable Studio 多 ship native atom，第三方 plugin 作者在 v1 substrate 上能走多远？ | 评估这套平台的 plugin 作者与集成方 |
| §23 | Readable Studio 自己的硬流程能不能在 plugin substrate 内重新表达？kernel 边界在哪？ | 确保没有特权后门的架构师与 reviewer |

...
export function formatUnsupportedNodeRuntimeMessage(runtime: NodeRuntimeDiagnosticInput = currentNodeRuntime()): string {
  return [
    `tools-dev must run with ${SUPPORTED_NODE_RANGE} before starting Readable Studio.`,
    ...
    "  corepack pnpm --filter @readable-studio/daemon rebuild better-sqlite3 --pending",
  ].join("\n");
}
...
function formatNativeAddonAbiMismatchRecommendation(runtime: NodeRuntimeDiagnosticInput): string {
  const base = [
    "Readable Studio's dev stack must run with Node ~24.",

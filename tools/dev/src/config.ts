...
function resolveAppConfig(options: {
  ...
  return {
    ...
    latestLogPath: resolveLogFilePath({ runtimeRoot: options.namespaceRoot, app: options.app, contract: SIDECAR_CONTRACT }),
    logDir: path.dirname(resolveLogFilePath({ runtimeRoot: options.namespaceRoot, app: options.app, contract: SIDECAR_CONTRACT })),
  };
}
...
export function resolveToolDevConfig(options: ToolDevOptions = {}): ToolDevConfig {
  ...
  const namespaceRoot = resolveNamespaceRoot({ base: toolsDevRoot, namespace, contract: SIDECAR_CONTRACT });
  ...
  return {
    apps: {
      ...
      web: {
        ...web,
        nextDistDir: resolveAppRuntimePath({ app: APP_KEYS.WEB, namespaceRoot, fileName: "next", contract: SIDECAR_CONTRACT }),
        nextTsconfigPath: resolveAppRuntimePath({ app: APP_KEYS.WEB, namespaceRoot, fileName: "tsconfig.json", contract: SIDECAR_CONTRACT }),

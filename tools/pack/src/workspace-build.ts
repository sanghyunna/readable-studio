...
const BUILD_COMMANDS = [
  { args: ["--filter", "@readable-studio/components", "build"] },
  { args: ["--filter", "@readable-studio/contracts", "build"] },
  { args: ["--filter", "@readable-studio/registry-protocol", "build"] },
  { args: ["--filter", "@readable-studio/sidecar-proto", "build"] },
  { args: ["--filter", "@readable-studio/sidecar", "build"] },
  { args: ["--filter", "@readable-studio/platform", "build"] },
  { args: ["--filter", "@readable-studio/download", "build"] },
  { args: ["--filter", "@readable-studio/host", "build"] },
  { args: ["--filter", "@readable-studio/agui-adapter", "build"] },
  { args: ["--filter", "@readable-studio/plugin-runtime", "build"] },
...
async function createWorkspaceBuildCacheKey(config: ToolPackConfig): Promise<string> {
  ...
  return hashJson({
    ...
    nativeBuild: config.platform === "win"
      ? { args: ["--filter", "@readable-studio/platform", "build:native:win32"] }
      : null,
    packageHashes,
...
function workspaceBuildArtifacts(config: ToolPackConfig): WorkspaceBuildArtifact[] {
  const artifacts = [
    ...
    "packages/product-identity/dist",

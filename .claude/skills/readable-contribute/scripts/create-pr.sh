#!/usr/bin/env bash
# Commit, push, and open a PR against sanghyunna/readable-studio.
# Usage: create-pr.sh --workdir <dir> --type <skill|design-system|i18n|docs> \
#                     --title "<pr title>" --body-file <rendered PR body .md>
#
# Reads:
#   <workdir>/.readable-contrib/contributor.txt   (display name; optional)
#   <workdir>/.readable-contrib/pitch.txt         (one-line pitch; optional)
...
[[ -n "$WORKDIR"   ]] || readable::die "--workdir required"
[[ -n "$TYPE"      ]] || readable::die "--type required (skill|design-system|i18n|docs)"
...
case "$BRANCH" in
  main|master|develop) readable::die "refusing to push base branch '$BRANCH'" ;;
esac
...
# the user's contribution PR: setup-workspace.sh adds `.readable-contrib/` to
# .git/info/exclude, but in case this script is invoked against a workdir
# set up differently, also pass `:!.readable-contrib` as a pathspec exclude so
# nothing under .readable-contrib/ gets staged here.
SCRATCH_EXCLUDE=':!:.readable-contrib'
if [[ -n "$(git status --porcelain -- . "$SCRATCH_EXCLUDE")" ]]; then
  ...
  if git diff --cached --quiet; then
    readable::log "no real changes after staging — skipping commit"
  else
    git commit -m "$TITLE"
    readable::log "created commit"
  fi
else
  readable::log "nothing new to commit (assuming work was already committed)"
fi
...
if [[ -n "${TARGET_FORK}" ]] && git remote | grep -q '^fork$'; then
  ...
  readable::warn "no fork configured (TARGET_FORK empty) — pushing to upstream ${TARGET_REPO}. 3s to abort..."
  sleep 3 || true
fi

readable::log "pushing to ${PUSH_REMOTE}/${BRANCH}"
git push -u "$PUSH_REMOTE" "$BRANCH"

# 3) Pick label set per contribution type. (Readable Studio's labels: documentation, i18n, blog, enhancement, ...)
LABELS=()
case "$TYPE" in
...
PR_URL="$(gh pr create \
  --repo "$TARGET_REPO" \
  --base "$READABLE_BASE_BRANCH" \
  --head "$HEAD_REF" \
  --title "$TITLE" \
  --body-file "$BODY_FILE" \
  ${DRAFT} \
  "${LABEL_FLAGS[@]}")" || readable::die "gh pr create failed"

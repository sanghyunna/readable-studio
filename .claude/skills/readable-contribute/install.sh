#!/usr/bin/env bash
# Readable Contribute installer — self-bootstrapping.
# Fetches the latest readable-contribute skill from sanghyunna/readable-studio and installs
...
#
#      curl -sSL https://raw.githubusercontent.com/sanghyunna/readable-studio/main/.claude/skills/readable-contribute/install.sh | bash
#
...
#
# Targets installed:
#   ~/.claude/skills/readable-contribute/        Claude Code (native skill format)
#   ~/.claude/commands/readable-contribute.md    Claude Code slash command
#   ~/.agents/skills/readable-contribute/        Codex CLI (canonical path)
#   ~/.codex/skills/readable-contribute/         Codex CLI (legacy, only if ~/.codex exists)
#
# Override the source branch with READABLE_CONTRIBUTE_BRANCH=feat/foo (default: main).

set -euo pipefail

REPO="sanghyunna/readable-studio"
BRANCH="${READABLE_CONTRIBUTE_BRANCH:-main}"
...
cyan "Installing Readable Contribute skill from ${REPO}@${BRANCH}..."
...
TARBALL_ROOT="readable-studio-${BRANCH//\//-}"
tar -xzf "$TARBALL" -C "$TMPDIR" \
  "${TARBALL_ROOT}/.claude/skills/readable-contribute" \
  "${TARBALL_ROOT}/.claude/commands/readable-contribute.md" \
  2>/dev/null || die "skill files not found in tarball — branch may have different layout"

SKILL_SRC="$TMPDIR/${TARBALL_ROOT}/.claude/skills/readable-contribute"
CMD_SRC="$TMPDIR/${TARBALL_ROOT}/.claude/commands/readable-contribute.md"
...
install_skill_to "$HOME/.claude/skills/readable-contribute" "Claude Code skill"
mkdir -p "$HOME/.claude/commands"
cp "$CMD_SRC" "$HOME/.claude/commands/readable-contribute.md"
green "  ✓ Claude Code slash command (/readable-contribute)"
gray  "      $HOME/.claude/commands/readable-contribute.md"

# --- Codex CLI (canonical) --------------------------------------------------
install_skill_to "$HOME/.agents/skills/readable-contribute" "Codex CLI skill (~/.agents/skills/)"

# --- Codex CLI (legacy) — only if user already has Codex --------------------
if [[ -d "$HOME/.codex" ]]; then
  install_skill_to "$HOME/.codex/skills/readable-contribute" "Codex CLI skill (legacy ~/.codex/skills/)"
fi
...
cat <<'EOF'
  ...
  In Claude Code:  type  /readable-contribute  in any chat.
  In Codex CLI:    type  @readable-contribute  or pick "Readable Studio — Contribute" from /skills.
  In other agents: ask the agent to follow ~/.claude/skills/readable-contribute/SKILL.md

The skill walks you through one of:

  * shipping a Skill or Design System you made with Readable Studio
  * translating a doc to a new language
  * fixing a typo or writing a use-case blog
  * reporting a clean bug

Need help? Readable Studio Discord:  https://discord.gg/qhbcCH8Am4
EOF

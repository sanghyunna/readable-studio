...
checked into the Readable Studio repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any Readable Studio agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for Readable Studio and
...
flow (including how Readable Studio behaves when Codex is not installed).

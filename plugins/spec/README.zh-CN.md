...
- [`PUBLISHING-REGISTRIES.zh-CN.md`](PUBLISHING-REGISTRIES.zh-CN.md) - 发布到 skills.sh、ClawHub、GitHub 和 Readable Studio 的策略。
...
4. 填写 `readable-studio.json`：`specVersion`、title、插件 `version`、tags、`readable.taskKind`、`readable.mode`、`readable.useCase.query`、`readable.pipeline`、inputs 和 capabilities。
...
包含 `SKILL.md` 的文件夹可以作为普通 skill 被兼容 Agent Skills 的客户端使用。添加 `readable-studio.json` 不应该降低可移植性；它只增加 Readable Studio 产品行为。

参考：

- Agent Skills overview: https://agentskills.io/home
- Agent Skills specification: https://agentskills.io/specification
- Readable Studio 插件完整 spec: ../../docs/plugins-spec.zh-CN.md

...
Readable Studio 插件刻意设计成一个文件夹可以跨多个 agent 生态流转。最稳妥的发布模型是：

1. 把公开 GitHub 仓库或 Readable Studio PR 作为 source of truth。
2. 保持 `SKILL.md` 可移植、适合 registry 读取。
3. 添加 `readable-studio.json` 作为 Readable Studio sidecar。
...
1. 本地验证插件文件夹。
2. 推送公开 GitHub 仓库，或向 Readable Studio 打开 PR。
3. 在 README 中添加 Readable Studio 和通用 Agent Skills 客户端的安装说明。
...
对 Readable Studio 插件作者：

- 确保 repo 或 subpath 包含合法 `SKILL.md`。
- 保持 `readable-studio.json` 为纯增量；通用 skill 客户端应能忽略它。
- 在 README 中放一个短安装块：
...
普通 Readable Studio 插件应优先走这个路径，因为它们以 `SKILL.md` 为核心。
...
- 链回 canonical GitHub repo 和 Readable Studio PR。
- changelog 诚实且版本化。
- `readable-studio.json` 的 `specVersion` 保持为规范包版本；每次可发布的行为变化都 bump 插件 `version`。

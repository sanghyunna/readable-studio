...
For Readable Studio plugin authors:

- Ensure the repo or subpath contains a valid `SKILL.md`.
- Keep `readable-studio.json` additive; generic skill clients should be able to ignore it.
...
Use this path for normal Readable Studio plugins because they are centered on `SKILL.md`.
...
- Link back to the canonical GitHub repo and Readable Studio PR.
- Keep changelog text honest and versioned.
- Keep `readable-studio.json` `specVersion` fixed to the spec kit version and bump plugin `version` for every publishable behavior change.

...
This directory owns Readable Studio plugin content and plugin authoring material.
...
- Keep runnable plugin examples portable: every example should have a `SKILL.md`; add `readable-studio.json` only as the Readable Studio sidecar.
- Keep `SKILL.md` bodies free of Readable Studio-only marketplace metadata. Put Readable Studio display, inputs, preview, pipeline, capabilities, and source information in `readable-studio.json`.
- Do not import app-private code from plugin content. A plugin may reference Readable Studio atoms, design systems, craft docs, assets, scripts, and MCP servers through the manifest.
...
- Use the v1 JSON schema at `docs/schemas/readable-studio.plugin.v1.json`.

---
name: readable-plugin-publish-github
description: Publish a local Readable Studio plugin to a new public GitHub repository using gh CLI.
triggers:
  - publish plugin
  - github repo
  - open source plugin
...
3. Call the local daemon endpoint instead of hand-rolling GitHub commands:
   `curl -sS -X POST "$READABLE_DAEMON_URL/api/projects/$READABLE_PROJECT_ID/plugins/publish-github" -H 'content-type: application/json' -d '{"path":"<plugin_context_path>"}'`

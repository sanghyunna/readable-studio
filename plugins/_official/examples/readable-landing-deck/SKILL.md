...
description: >
  ...
  dots and surreal collage plates).
...
triggers:
  ...
  - editorial slides
  - atelier zero deck
  - readable studio deck
  - readable studio landing deck
readable:
  category: brand-deck
  surface: web
  mode: deck
...
inputs:
  ...
  - id: deck_title
    ...
    description: e.g. `'Readable Studio · Vol. 01 / Issue Nº 26'`.
  - id: slides
  ...
  - id: imagery
    ...
    schema_path: ../readable-landing/schema.ts#ImageryConfig
parameters:
...
example_prompt: |
  ...
  card. Reuse the readable-landing image library.
---

# readable-landing-deck

Sister skill to [`readable-landing`](../readable-landing/). Same
...
```text
inputs.json + ../readable-landing/styles.css
        │
        └──────────► scripts/compose.ts
                            │
                            ▼
...
`readable-landing` brief, copy `brand` and `imagery` over verbatim.
...
script — it shares the 16-slot library from `readable-landing`. To
regenerate or stub:

```bash
# generate via gpt-image-2 (fal.ai)
FAL_KEY=... npx tsx ../readable-landing/scripts/imagegen.ts ../readable-landing/inputs.example.json --out=../readable-landing/assets/
...
live (default in the example: `../readable-landing/assets/`).
...
stylesheet from `../readable-landing/styles.css`, layers
...
- **Reuse the sister skill's stylesheet.** The composer reads
  `../readable-landing/styles.css` at compile time. Do not

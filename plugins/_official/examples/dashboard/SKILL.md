...
3. **Lay out** the required regions:
   - **Left sidebar** (220??60px): brand mark at top, 6?? nav links with
     icons, active state uses the DS accent.
   - **Top bar**: page title on the left, search input + user avatar / status
     on the right.
   - **Main**:
     - Row 1: 3?? KPI cards (label + big number + delta vs. prior period).
...
```
<artifact identifier="index" type="text/html" title="Dashboard Title">
<!doctype html>
<html>...</html>
</artifact>
```

...
3. **Lay out** three regions, expressed on the inline axis so the
   ...
   - **Inline-start nav** (240??80px, sticky): grouped link list, current
     page bolded with an `inline-start`-edge accent stripe. 3?? groups
     of 4?? links.
   ...
   - **Inline-end TOC** (200??40px, sticky): "On this page" with the
...
4. **Write** a single HTML document:
   ...
   - `data-readable-id` on the nav, article, and TOC.
5. **Prose**: write at least 350 words of believable docs. Include at least
   one shell command, one code snippet (5??5 lines), one callout, one table.
6. **Self-check**:
   - Body text wraps at the DS line-length sweet spot (60??5 chars).
...
```
<artifact identifier="index" type="text/html" title="Docs ??Page Title">
<!doctype html>
<html>...</html>
</artifact>
```

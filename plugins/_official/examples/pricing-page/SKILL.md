...
3. **Sections**, in order:
   ...
   2. **Plan cards** ??one card per tier. Each card: tier name, price (use the
      display font + larger scale for the number), 1-line positioning, 4??
      bullet features, primary CTA. Mark the recommended tier with the DS
      accent border or a small badge.
   3. **Comparison table** ??feature rows 횞 tier columns, ??/ ??/ value cells.
      Group features into 2?? logical sections (Core, Collaboration,
      Support, Security??. Sticky header.
   4. **FAQ** ??4?? collapsible Q&A items. Use `<details><summary>` for the
      collapse ??no JS.

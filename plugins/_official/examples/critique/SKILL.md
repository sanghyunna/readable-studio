...
A single self-contained HTML review report
including:
...
2. **Pasted HTML** — user pasted code in the chat: read it from the
   message.
3. **Generated by you in this turn** — you just created the document
   and want to self-critique: re-read the saved file, or the chat document in a no-file-tools run.
...
With file tools, save the review report once at the approved report path (`index.html` in a new report project). If the current entry is the document being reviewed, confirm a separate report destination first so the source stays intact. Finish with a brief reference to the saved report and a summary. Keep the user's topic in the document title.
...
```
<artifact identifier="index" type="text/html" title="Critique · <Artifact Title>">
<!doctype html>
<html>...</html>
</artifact>
```

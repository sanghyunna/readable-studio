...
readable:
  ...
  inputs:
    - name: theme
      ...
      default: helix
      values:
        - helix         # Modern minimal 쨌 light grey 쨌 ink + electric blue 쨌 SaaS metrics / board
        - holm          # Editorial serif 쨌 cream 쨌 ink + deep chestnut 쨌 legal / finance memo
        - vance         # Gallery 쨌 cream 쨌 cream serif on black bars 쨌 art catalog / showcase
        - bevel         # Y2K editorial 쨌 black 쨌 display type + product photo grid 쨌 campaign
        - world-dark    # Finance green dark 쨌 deep green 쨌 mint + neon yellow 쨌 policy report
        - world-mint    # Finance green light 쨌 mint 쨌 deep green + neon yellow 쨌 policy report
        - atlas         # Museum 쨌 black 쨌 serif + vermilion 쨌 long-form narrative / chapter deck
        - bluehouse     # Consumer card 쨌 deep navy 쨌 gradient cards + peach ??coral 쨌 product showcase
    - name: slide_count
      type: integer
      default: 6
      min: 3
      ...
      # at 3?? (see example-holm / example-bluehouse). Long-form chapters
      # (atlas) can run 8??2. Don't pad to hit the default.
---
...
?쒋?? assets/
??  ?붴?? template.html       ??seed: 8 themes via [data-theme=*], proven iframe-nav script (READ FIRST)
?쒋?? references/
??  ?쒋?? themes.md           ??8 themes: when-to-pick / do / don't / primary layouts
??  ?쒋?? layouts.md          ??10 paste-ready slide layouts, cross-theme
??  ?쒋?? components.md       ??shared primitives (eyebrow, kpi-row, image-grid, meta-bar)
??  ?붴?? checklist.md        ??P0/P1/P2 self-review + theme lock-in gate
?붴?? examples/               ??four reference decks across the most contrasting themes
    ?쒋?? example-helix.html       (SaaS board update 쨌 light minimal)
    ?쒋?? example-holm.html        (legal fintech memo 쨌 cream editorial serif)
    ?쒋?? example-atlas.html       (quarterly history chapter 쨌 black + vermilion)
    ?붴?? example-bluehouse.html   (real estate ROI 쨌 navy + gradient cards)
```
...
Run `references/checklist.md` silently before emit: the **P0 theme-lock gate** plus the five-dimension 1?? critique (Philosophy / Hierarchy / Execution / Specificity / Restraint). Any dimension ??3 ??re-do before emit.
...
- **Accent appears 1??횞 per slide max.** Never a gradient-spam.
...
| `magazine-web-ppt` | Editorial "magazine 횞 e-ink" aesthetic (WebGL fluid background, serif titles, chapter plates). When the brief asks for a keynote / launch / sharing-style deck and calls out Monocle / WIRED / Kinfolk / Domus. |

...
/**
 * Resolve the daemon HTTP base URL for `readable` client commands.
 *
 * Spawn order: explicit `--daemon-url` flag, `READABLE_DAEMON_URL` env, then
 ...
 * for direct `readable` launches that do not run as a sidecar.
 */
export async function resolveDaemonUrl(
  ...
  const envUrl = env.READABLE_DAEMON_URL;

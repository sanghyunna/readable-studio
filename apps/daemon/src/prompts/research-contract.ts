...
export function renderResearchCommandContract(
  ...
  const lines = [
    ...
    '',
    '```bash',
    `"$READABLE_NODE_BIN" "$READABLE_BIN" research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```powershell',
    `& $env:READABLE_NODE_BIN $env:READABLE_BIN research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```cmd',
    `"%READABLE_NODE_BIN%" "%READABLE_BIN%" research search --query "<search query>" --max-sources ${maxSources}`,
  ...
  if (safeQuery) {
    lines.push(
      ...
      'If the readable command fails because Tavily is not configured or unavailable, report the actual stderr/error, then use your own search capability as fallback and label the fallback clearly.',

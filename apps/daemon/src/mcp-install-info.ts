...
export interface BuildMcpInstallPayloadInputs {
   ...
   *  spawned `readable mcp` should discover the live URL via the IPC
  ...
  /** Browser-facing Readable Studio studio base URL (e.g.
...
export function buildMcpInstallPayload(
  ...
  if (!inputs.cliExists) {
    hints.push(
      `Readable Studio CLI entry is missing at ${inputs.cliPath}. Rebuild the daemon or packaged app and refresh.`,
    );
  }
  ...
  // Sidecar mode: omit --daemon-url so the spawned `readable mcp` discovers
  // the live URL via the IPC status socket on every spawn, surviving
  // ephemeral-port restarts. Direct `readable --port X` launches have no

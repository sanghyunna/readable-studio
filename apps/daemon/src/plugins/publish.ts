// Phase 4 / spec §14.1 — `readable plugin publish --to <catalog>` PR-template launcher.
//
  ...
  switch (args.catalog) {
    ...
    case 'readable-studio': {
      const bodyWithRegistry = [
        body,
        '',
        '## Readable Studio registry entry',
        '',
        '- Target path: `plugins/community/<plugin-name>/readable-studio.json`',
        '- Generated index: `plugins/registry/community/readable-studio-marketplace.json`',
        '- Required checks: `readable plugin validate`, `readable plugin pack`, integrity digest, preview smoke.',
      ].join('\n');
      const url = newIssueUrl('sanghyunna/readable-studio', title, bodyWithRegistry);
      return {
        catalog: args.catalog,
        catalogLabel: 'sanghyunna/readable-studio',
        url,
...
export function upsertMarketplaceJsonEntry(args: {
  ...
  const manifest: MarketplaceJsonManifest = {
    ...
    name: typeof existing.name === 'string' ? existing.name : 'readable-studio-marketplace',
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `readable-studio.json` sidecar (additive Readable Studio metadata).');
  lines.push('');
  lines.push('Generated by `readable plugin publish` — see https://github.com/sanghyunna/readable-studio/blob/main/docs/plugins-spec.md.');
  return lines.join('\n');
}

...
export { validateBaseUrl } from '@readable-studio/contracts/api/connectionTest';
...
function codexExecutableGuidance(
  ...
  return ` Configured Codex path failed: ${configuredOverridePath}. Readable Studio also detected a PATH Codex CLI at ${pathResolvedPath}. Update CODEX_BIN or clear the custom path to use the detected binary.`;
}
...
function attachAgentStreamHandlers(
  ...
  if (def.streamFormat === 'claude-stream-json') {
    ...
    acpSession = attachAcpSession({
      ...
      prompt,
      cwd,
      // Apply the same explicit-selection admission as a real chat run.
      // Omission stays null; catalog-only adapters reject uncatalogued ids.

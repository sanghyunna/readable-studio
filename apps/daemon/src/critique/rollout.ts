 ...
 * `READABLE_CRITIQUE_ENABLED=1` rather than relying on the client toggle,
 ...
 *      (`readable.critique.policy: required | opt-in | opt-out`).
 ...
 *   3. Environment override (`READABLE_CRITIQUE_ENABLED=1`). Useful for
 *      power users and CI fixtures.
 *   4. Global default. M0 / M1 = false. M2 = true for skills tagged
 *      `readable.critique.policy: required`. M3 = true globally.
 */
...
export interface RolloutInputs {
  /** Effective rollout phase. Reads from `READABLE_CRITIQUE_ROLLOUT_PHASE`
   *  in production; tests pass it directly. */
  phase: RolloutPhase;
  /** Skill's `readable.critique.policy` value, or `null` if the skill
   *  did not declare one. */
  ...
  /** Environment override (`READABLE_CRITIQUE_ENABLED=1`). */
...
/**
 * Parse the `READABLE_CRITIQUE_ROLLOUT_PHASE` env var into a `RolloutPhase`.
...
/**
 * Parse `READABLE_CRITIQUE_ENABLED`. Recognises the canonical truthy /

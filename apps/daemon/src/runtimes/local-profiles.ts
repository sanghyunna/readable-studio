...
function localAgentProfilesFile(): string | null {
  const explicit = process.env.READABLE_AGENT_PROFILES_CONFIG;
  const explicitPath =
  ...
  if (isSandboxModeEnabled(process.env)) {
    if (!process.env.READABLE_DATA_DIR?.trim()) return null;
  ...
  return path.join(homedir(), '.readable-studio', 'agents.local.json');
}

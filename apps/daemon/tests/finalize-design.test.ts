...
//
// Per memory `project_readable_studio_493_merged.md`: this file uses
...
function setupResolverFixture(): { db: any; projectsRoot: string } {
  tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'readable-finalize-'));
...
describe('finalizeDesignPackage (pipeline integration)', () => {
  function setupPipeline(
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'readable-finalize-pipe-'));
  ...
  // unconditionally so DESIGN.md landed in the hidden .readable-studio/projects/<id>
  ...
  it('writes DESIGN.md under metadata.baseDir for imported-folder projects', async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'readable-finalize-imported-'));
    const designSystemsRoot = path.join(tempDir, 'design-systems');
    fs.mkdirSync(designSystemsRoot, { recursive: true });
    ...
    // The user's actual folder lives outside .readable-studio/projects/.
    ...
    // `.readable-studio/projects/<id>` dir.
...
// `.readable-studio/projects/` via `path.join`. The HTTP layer happens to reject this

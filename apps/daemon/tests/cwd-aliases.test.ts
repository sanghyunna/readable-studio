...
function fresh(): string {
  return mkdtempSync(path.join(tmpdir(), 'readable-skill-stage-'));
}
...
describe('stageActiveSkill', () => {
  it('exposes the documented alias name so the skill preamble stays in sync', () => {
    expect(SKILLS_CWD_ALIAS).toBe('.readable-studio-skills');
  });

  it('stages a per-project copy under <cwd>/.readable-studio-skills/<folder>/', async () => {
    const fs = fresh();
  ...
  it('REGRESSION: writes through the staged copy do not mutate the source', async () => {
    ...
    // when `.readable-studio-skills` was a directory junction, an agent could

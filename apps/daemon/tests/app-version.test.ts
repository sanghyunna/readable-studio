...
describe('app version helpers', () => {
  ...
  it('prefers packaged app version metadata from the environment', () => {
    expect(resolveAppVersionInfo({
      ...
      env: { READABLE_APP_VERSION: '0.3.1-beta.1' },
      resourcesPath: '/Applications/Readable Studio.app/Contents/Resources',
      execPath: '/Applications/Readable Studio.app/Contents/Resources/readable-studio/bin/node',
      platform: 'darwin',
      arch: 'arm64',
    })).toEqual({
  ...
  it('detects packaged runtimes without sidecar protocol knowledge', () => {
    expect(isPackagedRuntime({ resourcesPath: '/Applications/Readable Studio.app/Contents/Resources' })).toBe(true);
    expect(isPackagedRuntime({
      execPath: '/Applications/Readable Studio.app/Contents/Resources/readable-studio/bin/node',
      platform: 'darwin',
    })).toBe(true);
    expect(isPackagedRuntime({
      execPath: 'C:\\Users\\Ada\\AppData\\Local\\Programs\\Readable Studio\\resources\\readable-studio\\bin\\node.exe',
      platform: 'win32',
    })).toBe(true);
    expect(isPackagedRuntime({
      execPath: '/opt/Readable Studio/resources/readable-studio/bin/node',
      platform: 'linux',
    })).toBe(true);
  ...
  it('honors an explicit release channel', () => {
    expect(resolveAppVersionInfo({
      packageMetadata: { version: '1.2.3' },
      env: { READABLE_RELEASE_CHANNEL: 'beta' },
    }).channel).toBe('beta');
  });

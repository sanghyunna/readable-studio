...
describe('project skillId validation', () => {
  ...
  describe('POST /api/projects', () => {
    ...
    it('accepts a valid bundled skill id and stores it as-is', async () => {
      ...
      expect(body.project.skillId).toBe('readable-landing');
    });
    ...
    it('canonicalizes an aliased skill id (editorial-collage → readable-landing)', async () => {
      const id = uniqueId('p');
      const resp = await createProject({
        id,
        name: 'Aliased skill',
  ...
  describe('PATCH /api/projects/:id', () => {
    ...
    it('normalizes empty-string skillId on patch to null', async () => {
      const id = uniqueId('p');
      await createProject({ id, name: 'Patch empty', skillId: 'readable-landing' });
      projectsToClean.push(id);
    ...
    it('treats null skillId on patch as unset', async () => {
      const id = uniqueId('p');
      await createProject({ id, name: 'Patch null', skillId: 'readable-landing' });
      projectsToClean.push(id);
    ...
    it('leaves skillId untouched when the field is omitted from patch', async () => {
      const id = uniqueId('p');
      await createProject({ id, name: 'Patch omit', skillId: 'readable-landing' });
      projectsToClean.push(id);

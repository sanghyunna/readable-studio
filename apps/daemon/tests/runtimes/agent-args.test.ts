...
test('opencode args keep run/json stdin delivery and select reasoning via --variant', () => {
  const prompt = 'design a dashboard';
  const baseArgs = opencode.buildArgs(prompt, [], [], {});
  assert.equal(opencode.promptViaStdin, true);
  assert.ok(opencode.reasoningOptions?.some((option) => option.id === 'high'));
  ...
  assert.deepEqual(withReasoning, [...withModel, '--variant', 'high']);

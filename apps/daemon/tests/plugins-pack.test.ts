...
beforeEach(async () => {
  parent = await mkdtemp(path.join(os.tmpdir(), 'readable-pack-'));
...
describe('packPlugin', () => {
  it('writes a .tgz containing every file in the folder', async () => {
    ...
    expect(result.files.sort()).toEqual([
      'SKILL.md', 'assets/logo.svg', 'readable-studio.json',
    ]);
  ...
  it('skips node_modules / .git / dist + .DS_Store noise', async () => {
    await writeFile(path.join(folder, 'readable-studio.json'), JSON.stringify({ name: 'p', version: '0.0.1' }));
    ...
    const result = await packPlugin({ folder });
    const entries = await listArchiveEntries(result.outPath);
    expect(entries).toEqual(['readable-studio.json']);
  });
  ...
  it('skips symlinks both at walk time and at filter time', async () => {
    ...
    const result = await packPlugin({ folder });
    expect(result.files).toEqual(['readable-studio.json', 'real.txt']);
    const entries = await listArchiveEntries(result.outPath);
    expect(entries).toEqual(['readable-studio.json', 'real.txt']);
  });

  it('falls back to <basename>.tgz when the manifest has no version', async () => {
    await writeFile(path.join(folder, 'readable-studio.json'), JSON.stringify({ name: 'plain' }));
    const result = await packPlugin({ folder });
  ...
  it('round-trips through tar.extract: archive contents match the source tree', async () => {
    await writeFile(path.join(folder, 'readable-studio.json'), JSON.stringify({ name: 'rt', version: '0.0.1' }));
    ...
    expect((await readFile(path.join(dest, 'readable-studio.json'), 'utf8')).includes('rt')).toBe(true);
  ...
  it('rejects a folder without readable-studio.json', async () => {
    await writeFile(path.join(folder, 'SKILL.md'), '---\nname: p\n---\n');
    await expect(packPlugin({ folder })).rejects.toBeInstanceOf(PackPluginError);
  });

  it('rejects a folder whose readable-studio.json is malformed JSON', async () => {
  ...
  it('does not pack the output archive itself when --out lands inside the folder', async () => {
    ...
    const result = await packPlugin({ folder, out });
    expect(result.files).toEqual(['a.txt', 'readable-studio.json']);
    const entries = await listArchiveEntries(result.outPath);
    expect(entries).toEqual(['a.txt', 'readable-studio.json']);
  });
});

// Contract test for the `readable templates` CLI surface. Keeps the
...
describe('readable templates CLI', () => {
  let stub: StubServer;
  ...
  it('prints usage on `readable templates help` and exits 0', async () => {
    const result = await runCli(['templates', 'help']);
    expect(result.code).toBe(0);
    expect(result.stdout).toMatch(/readable templates/);
  ...
  it('prints usage on `readable templates` with no sub-verb and exits 2', async () => {

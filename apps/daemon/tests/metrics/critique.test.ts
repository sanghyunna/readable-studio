...
const EXPECTED_SERIES = [
  { name: 'readable_studio_critique_runs_total', type: 'counter' as const },
  { name: 'readable_studio_critique_rounds_total', type: 'counter' as const },
  { name: 'readable_studio_critique_round_duration_ms', type: 'histogram' as const },
  { name: 'readable_studio_critique_composite_score', type: 'histogram' as const },
  { name: 'readable_studio_critique_must_fix_total', type: 'counter' as const },
  { name: 'readable_studio_critique_degraded_total', type: 'counter' as const },
  { name: 'readable_studio_critique_interrupted_total', type: 'counter' as const },
  { name: 'readable_studio_critique_parser_errors_total', type: 'counter' as const },
  { name: 'readable_studio_critique_protocol_version', type: 'gauge' as const },
];
...
describe('critique metrics registry (Phase 12)', () => {
  ...
  it('renders a happy-path bump into the exposition text', async () => {
    ...
    expect(text).toContain('readable_studio_critique_runs_total{status="shipped",adapter="mock",skill="unit-test"} 1');
    expect(text).toContain('readable_studio_critique_rounds_total{adapter="mock",skill="unit-test"} 1');
    expect(text).toContain('readable_studio_critique_must_fix_total{panelist="critic",dim="hierarchy",adapter="mock",skill="unit-test"} 1');
    expect(text).toContain('readable_studio_critique_degraded_total{reason="malformed_block",adapter="mock"} 1');
    expect(text).toContain('readable_studio_critique_interrupted_total{adapter="mock"} 1');
    expect(text).toContain('readable_studio_critique_parser_errors_total{kind="composite_mismatch",adapter="mock"} 1');
    expect(text).toContain('readable_studio_critique_protocol_version{version="1"} 1');
    // Histogram exposes _bucket / _sum / _count lines instead of a flat value.
    expect(text).toContain('readable_studio_critique_round_duration_ms_bucket{');
    expect(text).toContain('readable_studio_critique_composite_score_bucket{');
  });
  ...
  it('resets the registry between tests so cases stay isolated', async () => {
    ...
    __resetCritiqueMetricsForTests();
    const text = await getCritiqueMetrics();
    expect(text).not.toContain('readable_studio_critique_runs_total{status="shipped",adapter="a",skill="s"} 5');
  });
});

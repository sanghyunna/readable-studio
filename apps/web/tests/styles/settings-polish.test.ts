...
describe('settings polish CSS', () => {
  ...
  it('paints selected select options as a full-row state, not text-only emphasis', () => {
    const option = cssBlock(expandedIndexCss, '.readable-studio-select-option');
    const selected = cssBlock(expandedIndexCss, '.readable-studio-select-option.selected');
    const selectedHover = cssBlock(expandedIndexCss, '.readable-studio-select-option.selected:hover:not(:disabled),\n.readable-studio-select-option.selected.active:not(:disabled)');

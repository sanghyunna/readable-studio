...
// Behavioral coverage for nexu-io/readable-studio#890. The Examples / Tweaks
// preview iframe runs the bridge script generated by `buildSrcdoc`. When
// the agent emits an artifact without `data-readable-id` / `data-screen-label`
...
//      hint than "Click any element with `data-readable-id` …" — without
...
function extractBridgeScript(srcdoc: string): string {
  // The bridge is wrapped in `<script data-readable-selection-bridge>(function(){…})()</script>`.
  const match = srcdoc.match(
    /<script data-readable-selection-bridge>([\s\S]*?)<\/script>/,
  );
  if (!match || !match[1]) {
...
function extractSelectionBridgeStyle(srcdoc: string): string {
  const match = srcdoc.match(
    /<style data-readable-selection-bridge-style>[\s\S]*?<\/style>/,
  );
  return match?.[0] ?? '';
}
...
describe('selection bridge — empty annotation surface (#890)', () => {
  it('posts readable-studio:comment-targets with an empty list when the artifact has no annotated elements', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
      // PRD-style mockup: real DOM, zero `data-readable-id` / `data-screen-label`.
  ...
  it('does not post readable-studio:comment-target when the user clicks an unannotated element', async () => {
    ...
    // Click directly on the <h1> — it has no `data-readable-id` / `data-screen-label`,
  ...
  it('still posts readable-studio:comment-target when the click traverses up to an annotated ancestor', async () => {
    ...
    // to the no-annotation case. When ANY ancestor carries `data-readable-id`,
    // the click must still resolve to that ancestor — this is the
    // happy path the gallery has shipped since the bridge landed.
    const { win, parentPostMessage } = setupBridgeDom(
      '<main data-readable-id="hero"><h1 id="title">Hero</h1></main>',
      'inspect',
  ...
  it('does not add clickedDescendant when the annotated target itself is clicked', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
      '<main data-readable-id="hero">Hero</main>',
      'inspect',
      ['[data-readable-id="hero"]'],
    );
    ...
    win.document.querySelector('[data-readable-id="hero"]')!.dispatchEvent(
      new win.MouseEvent('click', { bubbles: true, cancelable: true }),
    );
  ...
  it('refreshes the active comment target when its text node mutates after picking it', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
      '<main data-readable-id="hero">Draft copy</main>',
      'comment',
      ['[data-readable-id="hero"]'],
    );
    ...
    const target = win.document.querySelector('[data-readable-id="hero"]');
    expect(target).not.toBeNull();
    target!.dispatchEvent(
      new win.MouseEvent('click', { bubbles: true, cancelable: true }),
    );
  ...
  it('uses Picker DOM fallback for meaningful unannotated elements on mixed pages', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
      '<main><section data-readable-id="hero">Hero</section><button id="cta">Launch</button></main>',
      'comment',
      ['#cta'],
    );
  ...
  it('does not broadcast the generated React root as a page-sized Comment target', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
      '<div id="root" data-readable-id="path-0"><header><a>Brand</a></header><main><h1 id="headline">Hero</h1></main><footer>Footer</footer></div>',
      'comment',
      ['#root', '#headline'],
    );
  ...
  it('applies pointer-events none to body iframes while Comment picker is active', async () => {
    const { win } = setupBridgeDom(
      '<article data-readable-id="card"><div>Tablet edition</div><iframe id="f" src="about:blank"></iframe></article>',
      'comment',
      ['#f'],
    );
    await new Promise<void>((resolve) => win.setTimeout(resolve, 10));
  ...
  it('applies pointer-events none to body iframes while Inspect picker is active', async () => {
    const { win } = setupBridgeDom(
      '<article data-readable-id="card"><iframe id="f" src="about:blank"></iframe></article>',
      'inspect',
      ['#f'],
    );
    await new Promise<void>((resolve) => win.setTimeout(resolve, 10));
  ...
  it('posts readable-studio:comment-target for the annotated card when the device-frame iframe is clicked', async () => {
    const { win, parentPostMessage } = setupBridgeDom(
      '<article data-readable-id="tablet-card" class="frame-card"><div class="meta">Tablet edition</div><iframe id="f" class="tablet-frame" title="Tablet edition" src="about:blank"></iframe></article>',
      'comment',
      ['#f', '.frame-card'],
    );
    await new Promise<void>((resolve) => win.setTimeout(resolve, 10));

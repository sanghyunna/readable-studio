...
describe('amrRechargeUrlForProfile', () => {
  it('matches the selected AMR profile wallet origin', () => {
    ...
    expect(amrRechargeUrlForProfile('local')).toBe(
      'http://localhost:5173/wallet?source=readable_studio',
    );
...
describe('resolveRunFailureUi', () => {
  it('never promotes the AMR switch card for non-AMR auth/quota errors', () => {
    // The promotional "switch to AMR & retry" nudge was removed; these codes
    // now fall through to a plain retry with no AMR upsell.
    for (const code of [
      'AGENT_AUTH_REQUIRED',
      'UNAUTHORIZED',
      'RATE_LIMITED',
    ...
    expect(resolveRunFailureUi('UNAUTHORIZED', null).showSwitchCard).toBe(false);
  });

...
describe('hub composer submit guard', () => {
  it('fires one create even when Start is pressed repeatedly', async () => {
    ...
    const completion = new Promise<boolean>((resolve) => { release = resolve; });
    const onSubmit = vi.fn(() => completion);
    render(
      <HubHome
        projects={PROJECTS}
    ...
    await act(async () => { release(true); await completion; });
    expect(send.disabled).toBe(true); // Accepted draft is now empty.
    expect(screen.getByTestId('home-hero-input').textContent).toBe('');
    setHomeHeroPrompt('Next complete draft');

...
describe('ChatComposer infinite re-render regression (#2097)', () => {
  ...
  it('restores a saved draft for the active conversation', async () => {
    window.localStorage.setItem('readable:chat-composer:draft:project-1:conv-1', 'draft before refresh');

    renderComposer({
      draftStorageKey: 'readable:chat-composer:draft:project-1:conv-1',
    });
    await flushMounts();

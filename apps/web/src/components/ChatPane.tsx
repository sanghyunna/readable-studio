...
import type { AppliedPluginSnapshot, ChatSessionMode, WorkspaceContextItem } from '@readable-studio/contracts';
...
//
// The default starter set stays i18n-translated via existing chat.example*
// keys so user-facing copy keeps its localizations.
type StarterPrompt = {
  icon: string;
  title: string;
  tag: string;
...
function pickStarters(t: TranslateFn): StarterPrompt[] {
...
export function ChatPane({
  ...
  useEffect(() => {
    ...
    // every failed-run guidance case (AMR auth/balance, Antigravity auth/quota,
    // upstream outage, generic retry).
    const key = [
      projectId ?? '',
      activeConversationId ?? '',
      retryAssistant.id,
...
export function buildRunErrorDiagnosticText(input: RunErrorDiagnosticInput): string {
  const lines = [
    'Readable Studio run error diagnostics',

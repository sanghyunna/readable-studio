...
const INSTALL_NOTE =
  'Prefer the supported `readable plugin install --source` flow after confirming the manifest.';
...
export function buildPluginFolderAgentActionPrompt(
  ...
  return [
    ...
    '',
    'Please do this through the `readable` CLI from the current project workspace, not through hidden UI APIs.',
    INSTALL_NOTE,
...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the Readable Studio community catalog at `sanghyunna/readable-studio`.',
    ...
    '',
    `\`"$READABLE_NODE_BIN" "$READABLE_BIN" plugin readable-studio-pr ${folderPath}\``,
    '',
    ...
    '- Do NOT call the legacy `readable plugin publish --to readable-studio` CLI — that flow produces an issue URL, which is the old path we are replacing.',
  ].join('\n');
}
...
// submission path — `readable plugin publish --to readable-studio` produces an
// Readable Studio issue URL and belongs to the "Readable Studio PR" button. Before
// this rewrite the prompt said "Use the supported `readable plugin publish` or
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/readable-studio.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `readable plugin publish --to readable-studio`. That command emits a Readable Studio issue URL and belongs to the "Readable Studio PR" button.',
    '',
    'Run this deterministic Readable Studio CLI workflow from the current project workspace:',
    '',
    `\`"$READABLE_NODE_BIN" "$READABLE_BIN" plugin publish-repo ${folderPath}\``,
    '',
    ...
    '',
    '**Hard constraints.** Treat these as inviolable:',
    '- Do NOT call `readable plugin publish --to readable-studio` (or any `--to <catalog>` variant). That is the registry-submission flow, not the repository-publish flow.',

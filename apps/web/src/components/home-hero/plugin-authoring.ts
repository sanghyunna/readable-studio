...
export const PLUGIN_AUTHORING_PROMPT_TEMPLATE = [
  ...
  '- readable-studio.json with valid metadata: specVersion, name, version, description, mode, task kind, inputs, plus any pipeline / context references the workflow needs',
  ...
  '',
  'Validate the plugin locally before reporting: run `readable plugin validate` on the folder, then `readable plugin pack` for a tarball, then `readable plugin install --source <absolute-folder-path>` to confirm the install path works.',
  '',
  'When the work above is done, write a single summary turn covering: files created, `readable plugin validate` status, local install / run status, and `readable plugin pack` output. Then STOP.',
  '',

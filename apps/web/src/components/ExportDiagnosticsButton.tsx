...
interface ReadableStudioDesktopApi {
  exportDiagnostics(): Promise<DesktopExportResult>;
}

declare global {
  interface Window {
    readableStudioDesktop?: ReadableStudioDesktopApi;
  }
}
 ...
 * save dialog via window.readableStudioDesktop) and the browser (triggers a
...
export function ExportDiagnosticsRow() {
  ...
  const handleClick = async () => {
    ...
    setStatus({ kind: 'busy' });
    try {
      if (window.readableStudioDesktop != null) {
        const result = await window.readableStudioDesktop.exportDiagnostics();
        if (result.ok) {

...
export const ChatComposer = forwardRef<ChatComposerHandle, Props>(
  function ChatComposer(
    ...
    // Persist the draft so a refresh/crash restores it. Debounced off the
    // typing hot path with eager flush points (clear, key change, unmount,
    // window hide) so durability matches the old per-keystroke sync write.
    // The returned imperative clear is for reset(): the submit path must
    // remove the stored draft synchronously, before React commits the ''
    // draft, or an unmount/pagehide in that gap resurrects the sent prompt.
    const clearPersistedDraftNow = useDebouncedDraftPersist(draftStorageKey, draft);
...
// Filters `prev` by `keep`, but returns the SAME array reference when nothing
// was removed. handleEditorChange re-runs this prune on every keystroke to keep
// staged chips in sync with the draft's @tokens; without an identity bailout
// `Array.prototype.filter` would hand React a fresh array each time, defeating
// its Object.is setState bailout and churning every memo keyed on the staged
// list (selectedWorkspaceContexts, the @-popover's filteredSkills) per
// character typed. Same precedent as appendOrderedStagedAttachments returning
// `current` when it adds nothing.
function prunedOrSame<T>(prev: T[], keep: (item: T) => boolean): T[] {
...
// How long a non-empty draft sits in memory before we persist it. Kept well
// under a second on purpose: durability used to come from a synchronous
// localStorage write on EVERY keystroke (a real typing-latency cost while a
// preview pane competes for the main thread), so this trades that per-character
// write for one trailing write per burst — but the existing "clears the saved
// draft after submitting it" spec polls localStorage on RTL's ~1s default, so
// the persisted text must land well within that window.
const DRAFT_PERSIST_DEBOUNCE_MS = 300;

// Debounced replacement for the old `useEffect(() => saveComposerDraft(key,
// draft))` that wrote synchronously per keystroke. The pending write carries
// its OWN key, so every flush path lands on the key that was live when the text
// was typed — which is what makes the flush-on-key-change land on the PREVIOUS
// conversation (§9.2 item 3) and what makes the immediate empty-draft flush
// after reset()→'' overwrite any stale pending write instead of resurrecting a
// sent draft (§9.2 item 1).
function useDebouncedDraftPersist(key: string | undefined, draft: string): () => void {
  const pendingRef = useRef<{ key: string; draft: string } | null>(null);
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  // Render-time mirror so clearNow — callable from event handlers, before any
  // effect of the same commit has run — always removes under the live key.
  const keyRef = useRef(key);
  keyRef.current = key;

  // Stable so the listener/cleanup effects below never re-subscribe: it reads
  // only refs, never render-scope state.
  const flush = useCallback(() => {
    ...
    saveComposerDraft(pending.key, pending.draft);
  }, []);

  useEffect(() => {
    if (!key) return; // No storage key → nothing to persist, mirroring saveComposerDraft's no-op.
    pendingRef.current = { key, draft };
    // Clearing the draft (send/reset, manual erase) is a removeItem we want
    // durable immediately: the synchronous flush also cancels any in-flight
    // timer, so a stale "type then send fast" write can't fire afterwards and
  ...
  // Imperative clear for the submit path. reset() must kill the stored draft
  // the moment the user sends — synchronously, inside the event handler. The
  // empty-draft flush above only runs when React commits the '' draft; in the
  // gap before that commit, an unmount (cleanup flush) or window close
  // (pagehide flush) would write the stale pending {key, draft} back and
  // resurrect the just-sent prompt. Dropping the pending entry and removing
  // the stored value here closes that gap; the later empty-draft effect run is
  // then an idempotent no-op removeItem.
  const clearNow = useCallback(() => {
    ...
    pendingRef.current = null;
    saveComposerDraft(keyRef.current, '');
  }, []);

  // Cleanup-only flush. Keyed on `key` (NOT draft) so it runs exactly on
  // unmount and on an in-place key change — never per keystroke, which would
  // re-sync the write. React runs this cleanup with pendingRef still holding
  // the OLD {key, draft}, so the flush lands on the previous conversation's key
  // before the schedule effect re-points the pending write at the new key.
  useEffect(() => flush, [key, flush]);

  // The packaged window can be closed/hidden with a debounced write still
  // pending; neither path unmounts React, so without these the trailing draft
  // is silently lost (invisible to jsdom tests). visibilitychange→hidden is the
  // reliable mobile/background-tab signal; pagehide covers desktop close.
  useEffect(() => {

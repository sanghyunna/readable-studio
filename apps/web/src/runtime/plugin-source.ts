...
const READABLE_STUDIO_REPO_URL = 'https://github.com/sanghyunna/readable-studio';
const READABLE_STUDIO_REPO_LABEL = 'sanghyunna/readable-studio';
...
export function derivePluginSourceLinks(
  ...
  const authorName = typeof author.name === 'string' && author.name.trim().length > 0
    ? author.name.trim()
    : null;
  const authorProfileUrl = officialBundled ? READABLE_STUDIO_REPO_URL : safeHttpUrl(author.url);
  const homepageUrl = officialBundled ? READABLE_STUDIO_REPO_URL : safeHttpUrl(homepageRaw);
  ...
  if (record.sourceKind === 'github') {
    ...
    sourceUrl = READABLE_STUDIO_REPO_URL;
    sourceLabel = READABLE_STUDIO_REPO_LABEL;
  } else {

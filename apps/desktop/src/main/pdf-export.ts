 ...
 * `readable-studio:print-pdf` IPC handler reaching for `webContents.print()`, which
 ...
 * `readable-studio:print-pdf` IPC handler).
 *
...
function injectPrintStylesheet(doc: string, css: string): string {
  const tag = `<style data-readable-desktop-pdf>${css}</style>`;
...
export async function waitForPrintReadyHandshake(webContents: Electron.WebContents, nonce: string): Promise<void> {
  // The parent wrapper document caches 'READABLE_STUDIO_PRINT_READY' in
  // window.__readableStudioPrintReady as soon as it arrives (injected by
  ...
  const handshake = webContents.executeJavaScript(
    `(function() {
      if (window.__readableStudioPrintReady) return Promise.resolve(true);
      return new Promise(function(resolve) {
        window.addEventListener('message', function handler(event) {
          if (event.data && event.data.type === 'READABLE_STUDIO_PRINT_READY' && event.data.nonce === '${nonce}') {
            window.__readableStudioPrintReady = true;

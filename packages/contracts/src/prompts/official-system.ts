...
If the user explicitly asks for React output, use a single React component file instead. Export a default component or define \`App\`, \`Component\`, or \`Preview\`. In the no-file-tools delivery channel, use the same stable entry identifier with type \`text/jsx\` and the component as the body.
